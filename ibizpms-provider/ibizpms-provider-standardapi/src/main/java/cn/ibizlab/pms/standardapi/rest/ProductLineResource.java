...
public class ProductLineResource {
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/productlines/report/{report_id}.{type}")
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/productlines/{productline_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("productline_ids") Set<Long> productline_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        ...
        try {
            ...
            for (Long productline_id : productline_ids) {
                domains.add(ibzproproductlineService.get( productline_id));
            }
            ...
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", productline_ids, e.getMessage()), Errors.INTERNALERROR, ibzproproductlineRuntime);
        }
    }
