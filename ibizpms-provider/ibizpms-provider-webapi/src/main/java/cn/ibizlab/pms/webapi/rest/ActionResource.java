...
public class ActionResource {
    ...
    @ApiOperation(value = "生成系统日志报表", tags = {"系统日志"}, notes = "生成系统日志报表")
    @RequestMapping(method = RequestMethod.GET, value = "/actions/report/{report_id}.{type}")
    public void report(@PathVariable("report_id") String report_id, @PathVariable("type") String type, ActionSearchContext context, HttpServletResponse response) {
        try {
            actionRuntime.outputReport(report_id, response.getOutputStream(), context, type, true);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", actionRuntime.getDEReportRuntime(report_id).getPSDEReport().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("生成报表[%s]发生错误：%s", report_id, e.getMessage()), Errors.INTERNALERROR, actionRuntime);
        }
    }

    @ApiOperation(value = "打印系统日志", tags = {"系统日志"}, notes = "打印系统日志")
    @RequestMapping(method = RequestMethod.GET, value = "/actions/{action_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("action_ids") Set<Long> action_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = actionRuntime.getDEPrintRuntime(print_id);
        try {
            List<Action> domains = new ArrayList<>();
            for (Long action_id : action_ids) {
                domains.add(actionService.get( action_id));
            }
            printRuntime.output(response.getOutputStream(), domains.toArray(new Action[domains.size()]), type);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", actionRuntime.getDEPrintRuntime(print_id).getPSDEPrint().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", action_ids, e.getMessage()), Errors.INTERNALERROR, actionRuntime);
        }
    }
