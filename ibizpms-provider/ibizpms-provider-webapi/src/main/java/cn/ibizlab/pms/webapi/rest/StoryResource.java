...
public class StoryResource {
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/stories/report/{report_id}.{type}")
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/stories/{story_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("story_ids") Set<Long> story_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = storyRuntime.getDEPrintRuntime(print_id);
        try {
            List<Story> domains = new ArrayList<>();
            for (Long story_id : story_ids) {
                domains.add(storyService.get( story_id));
            }
            ...
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", story_ids, e.getMessage()), Errors.INTERNALERROR, storyRuntime);
        }
    }
