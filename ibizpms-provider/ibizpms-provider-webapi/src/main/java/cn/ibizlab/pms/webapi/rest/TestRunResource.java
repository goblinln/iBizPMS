...
public class TestRunResource {
    ...
    @ApiOperation(value = "生成测试运行报表", tags = {"测试运行"}, notes = "生成测试运行报表")
    @RequestMapping(method = RequestMethod.GET, value = "/testruns/report/{report_id}.{type}")
    public void report(@PathVariable("report_id") String report_id, @PathVariable("type") String type, TestRunSearchContext context, HttpServletResponse response) {
        try {
            testrunRuntime.outputReport(report_id, response.getOutputStream(), context, type, true);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", testrunRuntime.getDEReportRuntime(report_id).getPSDEReport().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("生成报表[%s]发生错误：%s", report_id, e.getMessage()), Errors.INTERNALERROR, testrunRuntime);
        }
    }

    @ApiOperation(value = "打印测试运行", tags = {"测试运行"}, notes = "打印测试运行")
    @RequestMapping(method = RequestMethod.GET, value = "/testruns/{testrun_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("testrun_ids") Set<Long> testrun_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = testrunRuntime.getDEPrintRuntime(print_id);
        try {
            List<TestRun> domains = new ArrayList<>();
            for (Long testrun_id : testrun_ids) {
                domains.add(testrunService.get( testrun_id));
            }
            printRuntime.output(response.getOutputStream(), domains.toArray(new TestRun[domains.size()]), type);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", testrunRuntime.getDEPrintRuntime(print_id).getPSDEPrint().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", testrun_ids, e.getMessage()), Errors.INTERNALERROR, testrunRuntime);
        }
    }
