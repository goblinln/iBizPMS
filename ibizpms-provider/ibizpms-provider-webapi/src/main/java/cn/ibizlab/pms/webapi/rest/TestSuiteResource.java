...
public class TestSuiteResource {
    ...
    @ApiOperation(value = "生成测试套件报表", tags = {"测试套件"}, notes = "生成测试套件报表")
    @RequestMapping(method = RequestMethod.GET, value = "/testsuites/report/{report_id}.{type}")
    public void report(@PathVariable("report_id") String report_id, @PathVariable("type") String type, TestSuiteSearchContext context, HttpServletResponse response) {
        try {
            testsuiteRuntime.outputReport(report_id, response.getOutputStream(), context, type, true);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", testsuiteRuntime.getDEReportRuntime(report_id).getPSDEReport().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("生成报表[%s]发生错误：%s", report_id, e.getMessage()), Errors.INTERNALERROR, testsuiteRuntime);
        }
    }

    @ApiOperation(value = "打印测试套件", tags = {"测试套件"}, notes = "打印测试套件")
    @RequestMapping(method = RequestMethod.GET, value = "/testsuites/{testsuite_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("testsuite_ids") Set<Long> testsuite_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = testsuiteRuntime.getDEPrintRuntime(print_id);
        try {
            List<TestSuite> domains = new ArrayList<>();
            for (Long testsuite_id : testsuite_ids) {
                domains.add(testsuiteService.get( testsuite_id));
            }
            printRuntime.output(response.getOutputStream(), domains.toArray(new TestSuite[domains.size()]), type);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", testsuiteRuntime.getDEPrintRuntime(print_id).getPSDEPrint().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", testsuite_ids, e.getMessage()), Errors.INTERNALERROR, testsuiteRuntime);
        }
    }
