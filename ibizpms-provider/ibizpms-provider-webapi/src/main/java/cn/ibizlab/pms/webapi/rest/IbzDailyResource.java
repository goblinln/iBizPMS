...
public class IbzDailyResource {
    ...
    @PreAuthorize("test('IBZ_DAILY', #ibzdaily_id, 'NONE')")
    ...
    @PreAuthorize("quickTest('IBZ_DAILY', 'CREATE')")
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/ibzdailies/report/{report_id}.{type}")
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/ibzdailies/{ibzdaily_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("ibzdaily_ids") Set<Long> ibzdaily_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = ibzdailyRuntime.getDEPrintRuntime(print_id);
        try {
            List<IbzDaily> domains = new ArrayList<>();
            for (Long ibzdaily_id : ibzdaily_ids) {
                domains.add(ibzdailyService.get( ibzdaily_id));
            }
            ...
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", ibzdaily_ids, e.getMessage()), Errors.INTERNALERROR, ibzdailyRuntime);
        }
    }
