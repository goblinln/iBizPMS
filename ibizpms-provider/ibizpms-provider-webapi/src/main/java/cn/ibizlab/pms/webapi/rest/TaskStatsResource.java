...
public class TaskStatsResource {
    ...
    @ApiOperation(value = "生成任务统计报表", tags = {"任务统计"}, notes = "生成任务统计报表")
    @RequestMapping(method = RequestMethod.GET, value = "/taskstats/report/{report_id}.{type}")
    public void report(@PathVariable("report_id") String report_id, @PathVariable("type") String type, TaskStatsSearchContext context, HttpServletResponse response) {
        try {
            taskstatsRuntime.outputReport(report_id, response.getOutputStream(), context, type, true);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", taskstatsRuntime.getDEReportRuntime(report_id).getPSDEReport().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("生成报表[%s]发生错误：%s", report_id, e.getMessage()), Errors.INTERNALERROR, taskstatsRuntime);
        }
    }

    @ApiOperation(value = "打印任务统计", tags = {"任务统计"}, notes = "打印任务统计")
    @RequestMapping(method = RequestMethod.GET, value = "/taskstats/{taskstats_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("taskstats_ids") Set<Long> taskstats_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = taskstatsRuntime.getDEPrintRuntime(print_id);
        try {
            List<TaskStats> domains = new ArrayList<>();
            for (Long taskstats_id : taskstats_ids) {
                domains.add(taskstatsService.get( taskstats_id));
            }
            printRuntime.output(response.getOutputStream(), domains.toArray(new TaskStats[domains.size()]), type);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", taskstatsRuntime.getDEPrintRuntime(print_id).getPSDEPrint().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", taskstats_ids, e.getMessage()), Errors.INTERNALERROR, taskstatsRuntime);
        }
    }
