...
public class ReleaseResource {
    ...
    @PreAuthorize("quickTest('ZT_RELEASE', 'CREATE')")
    ...
    @PreAuthorize("test('ZT_RELEASE', #release_id, 'READ')")
    ...
    @PreAuthorize("test('ZT_RELEASE', #release_id, 'DELETE')")
    ...
    @PreAuthorize("quickTest('ZT_RELEASE', 'DELETE')")
    ...
    @PreAuthorize("test('ZT_RELEASE', #release_id, 'UPDATE')")
    ...
    @PreAuthorize("test('ZT_RELEASE', #release_id, 'ACTIVATE')")
    ...
    @PreAuthorize("test('ZT_RELEASE', #release_id, 'LINKBUG')")
    ...
    @PreAuthorize("test('ZT_RELEASE', #release_id, 'LINKSTORY')")
    ...
    @PreAuthorize("test('ZT_RELEASE', #release_id, 'NONE')")
    ...
    @PreAuthorize("test('ZT_RELEASE', #release_id, 'TERMINATE')")
    ...
    @PreAuthorize("quickTest('ZT_RELEASE', 'READ')")
    ...
    @ApiOperation(value = "生成发布报表", tags = {"发布"}, notes = "生成发布报表")
    @RequestMapping(method = RequestMethod.GET, value = "/releases/report/{report_id}.{type}")
    public void report(@PathVariable("report_id") String report_id, @PathVariable("type") String type, ReleaseSearchContext context, HttpServletResponse response) {
        try {
            releaseRuntime.outputReport(report_id, response.getOutputStream(), context, type, true);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", releaseRuntime.getDEReportRuntime(report_id).getPSDEReport().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("生成报表[%s]发生错误：%s", report_id, e.getMessage()), Errors.INTERNALERROR, releaseRuntime);
        }
    }

    @ApiOperation(value = "打印发布", tags = {"发布"}, notes = "打印发布")
    @RequestMapping(method = RequestMethod.GET, value = "/releases/{release_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("release_ids") Set<Long> release_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = releaseRuntime.getDEPrintRuntime(print_id);
        try {
            List<Release> domains = new ArrayList<>();
            for (Long release_id : release_ids) {
                domains.add(releaseService.get( release_id));
            }
            printRuntime.output(response.getOutputStream(), domains.toArray(new Release[domains.size()]), type);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", releaseRuntime.getDEPrintRuntime(print_id).getPSDEPrint().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", release_ids, e.getMessage()), Errors.INTERNALERROR, releaseRuntime);
        }
    }
    ...
    @PreAuthorize("test('ZT_RELEASE', 'ZT_PRODUCT', #product_id, 'READ', #release_id, 'READ')")
    ...
    @PreAuthorize("test('ZT_RELEASE', 'ZT_PRODUCT', #product_id, 'DELETE', #release_id, 'DELETE')")
    ...
    @PreAuthorize("test('ZT_RELEASE', 'ZT_PRODUCT', #product_id, 'UPDATE', #release_id, 'UPDATE')")
    ...
    @PreAuthorize("test('ZT_RELEASE', 'ZT_PRODUCT', #product_id, 'MANAGE', #release_id, 'ACTIVATE')")
    ...
    @PreAuthorize("test('ZT_RELEASE', 'ZT_PRODUCT', #product_id, 'MANAGE', #release_id, 'LINKBUG')")
    ...
    @PreAuthorize("test('ZT_RELEASE', 'ZT_PRODUCT', #product_id, 'MANAGE', #release_id, 'LINKSTORY')")
    ...
    @PreAuthorize("test('ZT_RELEASE', 'ZT_PRODUCT', #product_id, 'MANAGE', #release_id, 'TERMINATE')")
