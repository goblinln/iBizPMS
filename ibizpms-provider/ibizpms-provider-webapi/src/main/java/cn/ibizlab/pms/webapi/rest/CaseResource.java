...
public class CaseResource {
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/cases/report/{report_id}.{type}")
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/cases/{case_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("case_ids") Set<Long> case_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = caseRuntime.getDEPrintRuntime(print_id);
        try {
            List<Case> domains = new ArrayList<>();
            for (Long case_id : case_ids) {
                domains.add(caseService.get( case_id));
            }
            ...
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", case_ids, e.getMessage()), Errors.INTERNALERROR, caseRuntime);
        }
    }
    ...
    @PreAuthorize("test('ZT_CASE', 'ZT_PRODUCT', #product_id, 'READ', #case_id, 'FAVORITE')")
    ...
    @PreAuthorize("test('ZT_CASE', 'ZT_PRODUCT', #product_id, 'READ', #case_id, 'NFAVORITE')")
