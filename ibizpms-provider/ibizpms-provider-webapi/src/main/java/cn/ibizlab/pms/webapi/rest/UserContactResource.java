...
public class UserContactResource {
    ...
    @ApiOperation(value = "生成用户联系方式报表", tags = {"用户联系方式"}, notes = "生成用户联系方式报表")
    @RequestMapping(method = RequestMethod.GET, value = "/usercontacts/report/{report_id}.{type}")
    public void report(@PathVariable("report_id") String report_id, @PathVariable("type") String type, UserContactSearchContext context, HttpServletResponse response) {
        try {
            usercontactRuntime.outputReport(report_id, response.getOutputStream(), context, type, true);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", usercontactRuntime.getDEReportRuntime(report_id).getPSDEReport().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("生成报表[%s]发生错误：%s", report_id, e.getMessage()), Errors.INTERNALERROR, usercontactRuntime);
        }
    }

    @ApiOperation(value = "打印用户联系方式", tags = {"用户联系方式"}, notes = "打印用户联系方式")
    @RequestMapping(method = RequestMethod.GET, value = "/usercontacts/{usercontact_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("usercontact_ids") Set<Long> usercontact_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = usercontactRuntime.getDEPrintRuntime(print_id);
        try {
            List<UserContact> domains = new ArrayList<>();
            for (Long usercontact_id : usercontact_ids) {
                domains.add(usercontactService.get( usercontact_id));
            }
            printRuntime.output(response.getOutputStream(), domains.toArray(new UserContact[domains.size()]), type);
            response.setHeader("Content-Disposition", String.format("inline;filename=%1$s.%2$s", usercontactRuntime.getDEPrintRuntime(print_id).getPSDEPrint().getName(), type));
            response.setContentType(getContentType(type));
            response.setCharacterEncoding("utf-8");
        } catch (Exception e) {
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", usercontact_ids, e.getMessage()), Errors.INTERNALERROR, usercontactRuntime);
        }
    }
