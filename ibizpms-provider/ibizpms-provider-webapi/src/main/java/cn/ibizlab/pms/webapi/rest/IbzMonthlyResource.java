...
public class IbzMonthlyResource {
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/ibzmonthlies/report/{report_id}.{type}")
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/ibzmonthlies/{ibzmonthly_ids}/print/{print_id}.{type}")
    public void print(@PathVariable("ibzmonthly_ids") Set<Long> ibzmonthly_ids, @PathVariable("print_id") String print_id, @PathVariable("type") String type, HttpServletResponse response) {
        IDEPrintRuntime printRuntime = ibzmonthlyRuntime.getDEPrintRuntime(print_id);
        try {
            List<IbzMonthly> domains = new ArrayList<>();
            for (Long ibzmonthly_id : ibzmonthly_ids) {
                domains.add(ibzmonthlyService.get( ibzmonthly_id));
            }
            ...
            throw new DataEntityRuntimeException(String.format("打印数据[%s]发生错误：%s", ibzmonthly_ids, e.getMessage()), Errors.INTERNALERROR, ibzmonthlyRuntime);
        }
    }
