...
public class WebHeaderFilter extends ZuulFilter {
    ...
    public Object run() throws ZuulException {
        ...
        if (pathMatcher.match("/sysaccounts/**", requestURI)){
			ctx.addZuulRequestHeader("srfsystemid", env.getProperty("ibzuaa-api.system","f7ad7e05-9031-11eb-b882-00163e06e68c"));
            if (env.getProperty("ibiz.ref.service.ibzuaa-api.super", Boolean.class, false)) {
                String login = env.getProperty("ibzuaa-api.login", "login");
                String password = env.getProperty("ibzuaa-api.password", "password");
        ...
        if (pathMatcher.match("/ibizprotags/**", requestURI)){
			ctx.addZuulRequestHeader("srfsystemid", env.getProperty("pmspro-pluginserviceapi.system",""));
            if (env.getProperty("ibiz.ref.service.ibzuaa-api.super", Boolean.class, false)) {
                String login = env.getProperty("pmspro-pluginserviceapi.login", "login");
                String password = env.getProperty("pmspro-pluginserviceapi.password", "password");
                SuperLoginInterceptor superLoginInterceptor = new SuperLoginInterceptor(superLoginClient, login, password);
                ctx.addZuulRequestHeader("srfdcid", curUser.getSrfdcid());
                ctx.addZuulRequestHeader("Authorization", "Bearer " + superLoginInterceptor.getToken());
            }
