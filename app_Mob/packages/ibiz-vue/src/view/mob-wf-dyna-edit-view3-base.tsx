 ...
 */
export class MobWFDynaEditView3Base extends MainViewBase
    ...
    public async viewModelInit() {
        ...
        this.drtabInstance = ModelTool.findPSControlByName(
            'drtab',
    ...
    public renderToolBarInfo(){
        return (
        <van-action-sheet actions={this.linkModel} cancel-text="取消" class="wf-action-sheet" get-container="#app"  value={this.toolBarVisible} on-select={(item:any)=>{this.dynamic_toolbar_click(item)}} on-cancel={()=>{this.toolBarVisibleChange()}} on-click-overlay={()=>{this.toolBarVisibleChange()}}>
        </van-action-sheet>
        )
    }
        ...
        if (this.deDRTabPages && this.deDRTabPages.length > 0) {
            ...
            return [
                <van-tabs
                    ...
                    type="card"
                    class="workflow-tabs-container"
                >
                    <van-tab
                        tab={tabsName}
                        title={
                            this.drtabInstance.M?.editItemCaption
                                ? this.drtabInstance.M?.editItemCaption
                                : this.editFormInstance.logicName
                        }
                    >
                    ...
                    {this.deDRTabPages.map((deDRTabPage: IPSDEDRTabPage) => {
                        return (
                            <van-tab
                                tab={tabsName}
                                title={this.$tl(
                                    deDRTabPage.getCapPSLanguageRes()
                                        ?.lanResTag,
                                    deDRTabPage.caption
                                )}
                            >
