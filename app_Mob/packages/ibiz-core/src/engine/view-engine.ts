...
export class ViewEngine {
    ...
    public calcToolbarItemAuthState(data: any) {
        ...
        if (!_this.view.appUIService)
            return;
        for (const _key in _this.view.toolbarModels) {
            if (Object.prototype.hasOwnProperty.call(_this.view.toolbarModels, _key)) {
                const _item = _this.view.toolbarModels[_key];
                    if (_item && _item['dataaccaction']) {
                        ...
                        if (dataActionResult === 0) {
                            ...
                            if ((_item.noprivdisplaymode === 2) || (_item.noprivdisplaymode === 6)) {
                                _this.view.toolbarModels[_key].visabled = false;
                            } else {
                                _this.view.toolbarModels[_key].visabled = true;
                            }
                        }
                        if (dataActionResult === 1) {
                            _this.view.toolbarModels[_key].visabled = true;
                            _this.view.toolbarModels[_key].disabled = false;
                        }
                    }
            }
        }
