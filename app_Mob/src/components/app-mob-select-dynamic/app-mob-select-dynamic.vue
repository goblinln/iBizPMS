    ...
    <div class="app-mobile-select" data-tap-disabled="true">
        ...
        <ion-select  :value="curValue" :disabled="disabled ? disabled : false" @ionChange="change" interface="action-sheet" @click="load" :cancel-text="$t('app.button.cancel')" @ionCancel="cancel">
              <template v-if="codeListType == 'DYNAMIC'">
                <ion-select-option  v-for="option of cachoptions" :key="option.value" :value="option.value" class="mob-select-text">{{($t('userCustom.'+tag+'.'+option.value)!== ('userCustom.'+tag+'.'+option.value))?$t('userCustom.'+tag+'.'+option.value) : option.text}}</ion-select-option>
              </template>
              <template v-else>
                <ion-select-option  v-for="option of cachoptions" :key="option.value" :value="option.value" class="mob-select-text">{{($t('codelist.'+tag+'.'+option.value)!== ('codelist.'+tag+'.'+option.value))?$t('codelist.'+tag+'.'+option.value) : option.text}}</ion-select-option>
              </template>
        </ion-select>
    </div>   
</template>
...
export default class AppSelect extends Vue {
     ...
     * @memberof AppSelect
     */
    @Prop({ default: ()=>{return {}} }) protected navigateParam?: any;
    ...
    public async load(): Promise<any> {
        ...
        if (response) {
            ...
            if(this.cachoptions.length == 0){
               this.cachoptions = response;
            }
...
<style lang="less">
@import './app-mob-select-dynamic.less';
</style>