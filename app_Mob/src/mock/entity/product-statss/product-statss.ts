...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/ibizpms-app-mob\/productstats\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/ibizpms-app-mob\/productstats\/process-definitions-nodes$/)).reply((config: any) => {
...
// FetchNoOpenProduct
mock.onGet(new RegExp(/^\/productstats\/fetchnoopenproduct$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FetchNoOpenProduct");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchNoOpenProduct
mock.onGet(new RegExp(/^\/productstats\/fetchnoopenproduct(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FetchNoOpenProduct");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
...
mock.onGet(new RegExp(/^\/productstats\/fetchprodctquantigird$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FetchProdctQuantiGird");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchProdctQuantiGird
mock.onGet(new RegExp(/^\/productstats\/fetchprodctquantigird(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FetchProdctQuantiGird");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
...
mock.onGet(new RegExp(/^\/productstats\/fetchproductinputtable$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FetchProductInputTable");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchProductInputTable
mock.onGet(new RegExp(/^\/productstats\/fetchproductinputtable(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FetchProductInputTable");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
...
mock.onGet(new RegExp(/^\/productstats\/fetchproductcompletionstatistics$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FetchProductcompletionstatistics");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchProductcompletionstatistics
mock.onGet(new RegExp(/^\/productstats\/fetchproductcompletionstatistics(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FetchProductcompletionstatistics");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
...
// FilterUpdate
mock.onPost(new RegExp(/^\/productstats\/?([a-zA-Z0-9\-\;]{0,35})\/fetchproductcompletionstatistics$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FilterUpdate");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const paramArray:Array<any> = ['id'];
    const matchArray:any = new RegExp(/^\/productstats\/([a-zA-Z0-9\-\;]{1,35})\/fetchproductcompletionstatistics$/).exec(config.url);
    let tempValue: any = {};
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
...
// FilterSearch
mock.onPost(new RegExp(/^\/productstats\/?([a-zA-Z0-9\-\;]{0,35})\/fetchproductcompletionstatistics$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FilterSearch");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterGet
mock.onPost(new RegExp(/^\/productstats\/?([a-zA-Z0-9\-\;]{0,35})\/fetchproductcompletionstatistics$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FilterGet");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterCreate
mock.onPost(new RegExp(/^\/productstats\/?([a-zA-Z0-9\-\;]{0,35})\/fetchproductcompletionstatistics$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FilterCreate");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterGetDraft
mock.onPost(new RegExp(/^\/productstats\/?([a-zA-Z0-9\-\;]{0,35})\/fetchproductcompletionstatistics$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FilterGetDraft");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterRemove
mock.onPost(new RegExp(/^\/productstats\/?([a-zA-Z0-9\-\;]{0,35})\/fetchproductcompletionstatistics$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FilterRemove");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterFetch
mock.onPost(new RegExp(/^\/productstats\/?([a-zA-Z0-9\-\;]{0,35})\/fetchproductcompletionstatistics$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: FilterFetch");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// GetTestStats
mock.onGet(new RegExp(/^\/productstats\/([a-zA-Z0-9\-\;]{1,35})\/getteststats$/)).reply((config: any) => {
    console.groupCollapsed("实体:productstats 方法: GetTestStats");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const paramArray:Array<any> = ['id'];
    const matchArray:any = new RegExp(/^\/productstats\/([a-zA-Z0-9\-\;]{1,35})\/getteststats$/).exec(config.url);
    let tempValue: any = {};
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
