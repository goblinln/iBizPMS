...
public class IBZProProductRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime {
    ...
    public List<IBZProProduct> selectDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        ...
        if (iPSDataQuery.getName().equals("DEFAULT"))
            return ibzproproductService.selectQueryByDefault(searchContext);
        if (iPSDataQuery.getName().equals("VIEW"))
            return ibzproproductService.selectQueryByView(searchContext);
        return null;
    }
    ...
    public IBZProProduct selectOne(ISearchContextBase iSearchContextBase) {
        ...
        List<IBZProProduct> domains = ibzproproductService.select(searchContext);
        if (domains.size() == 0)
            return null;
