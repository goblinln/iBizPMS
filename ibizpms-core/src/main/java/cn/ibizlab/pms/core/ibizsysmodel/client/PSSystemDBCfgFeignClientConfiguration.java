...
/**
 * 实体[PSSystemDBCfg] 服务对象接口配置
 */
@Configuration
public class PSSystemDBCfgFeignClientConfiguration {
    ...
    @Bean
    public PSSystemDBCfgFeignClient pSSystemDBCfgFeignClient(Decoder decoder, Encoder encoder, Client client, Contract contract, List<RequestInterceptor> requestInterceptors) {
        ...
        if (builder instanceof HystrixFeign.Builder) {
            ...
            return nameBuilder.target(PSSystemDBCfgFeignClient.class, "http://" + serviceValue, new PSSystemDBCfgFallback());
        } else {
            Feign.Builder nameBuilder = Feign.builder()
                    ...
                    .contract(contract)
                    .requestInterceptors(requestInterceptors);
            return nameBuilder.target(PSSystemDBCfgFeignClient.class, "http://" + serviceValue);
        }
    }
}
