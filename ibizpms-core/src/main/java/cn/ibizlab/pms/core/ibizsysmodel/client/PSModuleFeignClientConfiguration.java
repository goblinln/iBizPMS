...
/**
 * 实体[PSModule] 服务对象接口配置
 */
@Configuration
public class PSModuleFeignClientConfiguration {
    ...
    @Bean
    public PSModuleFeignClient pSModuleFeignClient(Decoder decoder, Encoder encoder, Client client, Contract contract, List<RequestInterceptor> requestInterceptors) {
        ...
        if (builder instanceof HystrixFeign.Builder) {
            ...
            return nameBuilder.target(PSModuleFeignClient.class, "http://" + serviceValue, new PSModuleFallback());
        } else {
            Feign.Builder nameBuilder = Feign.builder()
                    ...
                    .contract(contract)
                    .requestInterceptors(requestInterceptors);
            return nameBuilder.target(PSModuleFeignClient.class, "http://" + serviceValue);
        }
    }
}
