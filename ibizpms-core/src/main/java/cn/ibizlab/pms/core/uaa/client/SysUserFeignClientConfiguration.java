...
public class SysUserFeignClientConfiguration {
    ...
    @Bean
    public SysUserFeignClient sysUserFeignClient(Decoder decoder, Encoder encoder, Client client, Contract contract, List<RequestInterceptor> requestInterceptors) {
        ...
        if (builder instanceof HystrixFeign.Builder) {
            ...
            return nameBuilder.target(SysUserFeignClient.class, "http://" + serviceValue, new SysUserFallback());
        } else {
            Feign.Builder nameBuilder = Feign.builder()
                    ...
                    .contract(contract)
                    .requestInterceptors(requestInterceptors);
            return nameBuilder.target(SysUserFeignClient.class, "http://" + serviceValue);
        }
    }
}
