...
public class TaskHelper extends ZTBaseHelper<TaskMapper, Task> {
    ...
    public boolean create(Task et) {
        ...
        actionHelper.sendTodo(et.getId(), et.getName(), noticeusers, et.getAssignedto(), et.getMailto(), "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", StaticDict.Action__type.OPENED.getText());
    ...
    public void judChanges(List<History> changes, Task et, String noticeusers, String comment) {
        ...
        actionHelper.sendToread(et.getId(), et.getName(), noticeusers, et.getAssignedto(),
                et.getMailto(), "任務", StaticDict.Action__object_type.TASK.getValue(),
               "tasks", strActionText);
...
//        List<JSONObject> list = taskService.select(sql, null);
    ...
    public Task start(Task et) {
        ...
        if (changes.size() > 0 || StringUtils.isNotBlank(comment)) {
            ...
            actionHelper.sendToread(newTask.getId(), newTask.getName(), noticeusers, newTask.getAssignedto(),
                    newTask.getMailto(), "任務", StaticDict.Action__object_type.TASK.getValue(),
                   "tasks", strActionText);
    ...
    public Task restart(Task et) {
        ...
        if (changes.size() > 0 || StringUtils.isNotBlank(comment)) {
            ...
            actionHelper.sendToread(newTask.getId(), newTask.getName(), noticeusers, newTask.getAssignedto(), newTask.getMailto(),
                    "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", strActionText);
            Action action = actionHelper.create(StaticDict.Action__object_type.TASK.getValue(), newTask.getId(), strAction,
                    comment, "", null, true);
            if (changes.size() > 0) {
                actionHelper.logHistory(action.getId(), changes);
    ...
    public Task pause(Task et) {
        ...
        if (changes.size() > 0 || StringUtils.isNotBlank(comment)) {
            actionHelper.sendToread(newTask.getId(), newTask.getName(), noticeusers, newTask.getAssignedto(), newTask.getMailto(),
                    "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", StaticDict.Action__type.PAUSED.getText());
...
//        taskEstimateHelper.create(taskEstimate);
        ...
        if (changes.size() > 0 || StringUtils.isNotBlank(comment)) {
            if (jugAssignToIsChanged(old, et)) {
                actionHelper.sendMarkDone(newTask.getId(), newTask.getName(), old.getAssignedto(), "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", StaticDict.Action__type.ASSIGNED.getText());
                actionHelper.sendTodo(newTask.getId(), newTask.getName(), noticeusers, newTask.getAssignedto(), newTask.getMailto(),
                        "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", StaticDict.Action__type.ASSIGNED.getText());
            }
    ...
    public Task activate(Task et) {
        ...
        if (changes.size() > 0 || StringUtils.isNotBlank(comment)) {
            actionHelper.sendTodo(newTask.getId(), newTask.getName(), noticeusers, newTask.getAssignedto(), newTask.getMailto(),
                    "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", StaticDict.Action__type.ACTIVATED.getText());
    ...
    public Task cancel(Task et) {
        ...
        if (changes.size() > 0 || StringUtils.isNotBlank(comment)) {
            actionHelper.sendToread(newTask.getId(), newTask.getName(), noticeusers, newTask.getAssignedto(), newTask.getMailto(),
                    "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", StaticDict.Action__type.CANCELED.getText());
    ...
    public Task finish(Task et) {
        ...
        List<History> changes = ChangeUtil.diff(old, newTask);
        if (changes.size() > 0 || StringUtils.isNotBlank(comment)) {
            actionHelper.sendMarkDone(newTask.getId(), newTask.getName(), old.getAssignedto(), "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", StaticDict.Action__type.FINISHED.getText());
            actionHelper.sendTodo(newTask.getId(), newTask.getName(), noticeusers, newTask.getAssignedto(), newTask.getMailto(),
                    "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", StaticDict.Action__type.FINISHED.getText());
    ...
    public Task close(Task et) {
        ...
        if (changes.size() > 0 || StringUtils.isNotBlank(comment)) {
            actionHelper.sendToread(newTask.getId(), newTask.getName(), noticeusers, newTask.getAssignedto(), newTask.getMailto(),
                    "任務", StaticDict.Action__object_type.TASK.getValue(),"tasks", StaticDict.Action__type.CLOSED.getText());
