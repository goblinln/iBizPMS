...
public class PSSysSFPubFeignClientConfiguration {
    ...
    @Bean
    public PSSysSFPubFeignClient pSSysSFPubFeignClient(Decoder decoder, Encoder encoder, Client client, Contract contract, List<RequestInterceptor> requestInterceptors) {
        ...
        if (builder instanceof HystrixFeign.Builder) {
            ...
            return nameBuilder.target(PSSysSFPubFeignClient.class, "http://" + serviceValue, new PSSysSFPubFallback());
        } else {
            Feign.Builder nameBuilder = Feign.builder()
                    ...
                    .contract(contract)
                    .requestInterceptors(requestInterceptors);
            return nameBuilder.target(PSSysSFPubFeignClient.class, "http://" + serviceValue);
        }
    }
}
