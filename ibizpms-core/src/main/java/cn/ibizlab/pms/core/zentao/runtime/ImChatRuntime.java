...
public class ImChatRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime {
    ...
    public List<ImChat> selectDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        ...
        if (iPSDataQuery.getName().equals("DEFAULT"))
            return imchatService.selectQueryByDefault(searchContext);
        if (iPSDataQuery.getName().equals("VIEW"))
            return imchatService.selectQueryByView(searchContext);
        return null;
    }
    ...
    public ImChat selectOne(ISearchContextBase iSearchContextBase) {
        ...
        List<ImChat> domains = imchatService.select(searchContext);
        if (domains.size() == 0)
            return null;
        ...
        if (iPSDEAction != null) {
            if (iPSDEAction.getName().equals("Create")) {
                ...
                if(args[0] instanceof ImChat){
                    ...
                    arg = imchatService.get(arg.getId()) ;
                    return arg;
                }else{
