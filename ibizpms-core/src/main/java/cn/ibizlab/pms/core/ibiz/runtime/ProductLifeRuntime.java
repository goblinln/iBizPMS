...
public class ProductLifeRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime {
    ...
    public boolean existsData(ISearchContextBase iSearchContextBase) {
        Page<ProductLife> domains = productlifeService.searchDefault((ProductLifeSearchContext) iSearchContextBase);
    ...
    public Page<ProductLife> searchDataSet(IPSDEDataSet iPSDEDataSet, ISearchContextBase iSearchContextBase) {
        if (iPSDEDataSet.getName().equals("DEFAULT"))
            return productlifeService.searchDefault((ProductLifeSearchContext) iSearchContextBase);
        return null;
    }

    @Override
    public Page<ProductLife> searchDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        //暂未实现
        return null;
    }

    @Override
    public ProductLife selectOne(ISearchContextBase iSearchContextBase) {
        //单条数据查询，多条数数据时 返回第一条
        Page<ProductLife> domains = productlifeService.searchDefault((ProductLifeSearchContext) iSearchContextBase);
        if (domains.getSize() == 0)
            return null;
    ...
    public List<ProductLife> select(ISearchContextBase iSearchContextBase) {
        return productlifeService.searchDefault((ProductLifeSearchContext) iSearchContextBase).getContent();
    }
