...
/**
 * 实体[PSSubSysServiceAPI] 服务对象接口配置
 */
@Configuration
public class PSSubSysServiceAPIFeignClientConfiguration {
    ...
    @Bean
    public PSSubSysServiceAPIFeignClient pSSubSysServiceAPIFeignClient(Decoder decoder, Encoder encoder, Client client, Contract contract, List<RequestInterceptor> requestInterceptors) {
        ...
        if (builder instanceof HystrixFeign.Builder) {
            ...
            return nameBuilder.target(PSSubSysServiceAPIFeignClient.class, "http://" + serviceValue, new PSSubSysServiceAPIFallback());
        } else {
            Feign.Builder nameBuilder = Feign.builder()
                    ...
                    .contract(contract)
                    .requestInterceptors(requestInterceptors);
            return nameBuilder.target(PSSubSysServiceAPIFeignClient.class, "http://" + serviceValue);
        }
    }
}
