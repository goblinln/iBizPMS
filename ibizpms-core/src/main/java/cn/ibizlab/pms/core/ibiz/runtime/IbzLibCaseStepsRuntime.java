...
public class IbzLibCaseStepsRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime {
    ...
    public List<IbzLibCaseSteps> selectDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        ...
        if (iPSDataQuery.getName().equals("DEFAULT"))
            return ibzlibcasestepsService.selectQueryByDefault(searchContext);
        if (iPSDataQuery.getName().equals("VIEW"))
            return ibzlibcasestepsService.selectQueryByView(searchContext);
        return null;
    }
    ...
    public IbzLibCaseSteps selectOne(ISearchContextBase iSearchContextBase) {
        ...
        List<IbzLibCaseSteps> domains = ibzlibcasestepsService.select(searchContext);
        if (domains.size() == 0)
            return null;
        ...
        if (iPSDEAction != null) {
            if (iPSDEAction.getName().equals("Create")) {
                ...
                if(args[0] instanceof IbzLibCaseSteps){
                    ...
                    arg = ibzlibcasestepsService.get(arg.getId()) ;
                    return arg;
                }else{
