...
public class StorySpecModelImpl extends DataEntityModelImpl {
    ...
    public boolean test(Serializable key, String action) {
        ...
        List<StorySpec> domains = storyspecService.searchDefault(context).getContent();
        if (domains.size() == 0) {
            return false;
        }
        try {
            return SpringContextHolder.getBean(cn.ibizlab.pms.core.zentao.runtime.StorySpecRuntime.class).testDataAccessAction(domains.get(0),action);
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean test(List<Serializable> keys, String action) {
        ...
        try {
            for(StorySpec domain : domains){
                if(SpringContextHolder.getBean(cn.ibizlab.pms.core.zentao.runtime.StorySpecRuntime.class).testDataAccessAction(domain,action)){
                    return false;
                }
            }
        } catch (Exception e) {
