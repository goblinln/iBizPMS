...
@Component
@Slf4j
public class IBZProStoryModuleRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime implements IIBZProStoryModuleRuntime {
    ...
    public IEntityBase deserializeEntity(Object objData) {
        try {
            return MAPPER.readValue(String.valueOf(objData),IBZProStoryModule.class);
        } catch (Exception e) {
    ...
    public Page<IBZProStoryModule> searchDataSet(IPSDEDataSet iPSDEDataSet, ISearchContextBase iSearchContextBase) {
        IBZProStoryModuleSearchContext searchContext = (IBZProStoryModuleSearchContext) iSearchContextBase;
        if (iPSDEDataSet.getName().equals("DEFAULT"))
            return ibzprostorymoduleService.searchDefault(searchContext);    
        return null;
    }

    @Override
    public List<IBZProStoryModule> selectDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        IBZProStoryModuleSearchContext searchContext = (IBZProStoryModuleSearchContext) iSearchContextBase;
        if (iPSDataQuery.getName().equals("DEFAULT"))
            return ibzprostorymoduleService.selectDefault(searchContext);
        if (iPSDataQuery.getName().equals("VIEW"))
            return ibzprostorymoduleService.selectView(searchContext);
        return null;
    }
    ...
    public List<IBZProStoryModule> select(ISearchContextBase iSearchContextBase) {
        IBZProStoryModuleSearchContext searchContext = (IBZProStoryModuleSearchContext) iSearchContextBase;
        return ibzprostorymoduleService.select(searchContext);
    }
    ...
    public Object executeAction(String strActionName, IPSDEAction iPSDEAction, Object[] args) throws Throwable {
        if (iPSDEAction != null) {
            ...
            if (strActionName.equalsIgnoreCase("Create")) {
                ...
                if(args[0] instanceof IBZProStoryModule){
                    IBZProStoryModule arg = (IBZProStoryModule) args[0] ;
                    CachedBeanCopier.copy(ibzprostorymoduleService.sysGet(arg.getId()), arg);
                    return arg;
                }else{
                    ...
                    return ibzprostorymoduleService.sysUpdate((IBZProStoryModule) args[0]);
            }             
        }
