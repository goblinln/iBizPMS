...
@Component
@Slf4j
public class ProjectTaskestimateRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime implements IProjectTaskestimateRuntime {
    ...
    public IEntityBase deserializeEntity(Object objData) {
        try {
            return MAPPER.readValue(String.valueOf(objData),ProjectTaskestimate.class);
        } catch (Exception e) {
    ...
    public List<ProjectTaskestimate> selectDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        ...
        if (iPSDataQuery.getName().equals("AccountDetail"))
            return projecttaskestimateService.selectAccountDetail(searchContext);
        if (iPSDataQuery.getName().equals("DEFAULT"))
            return projecttaskestimateService.selectDefault(searchContext);
        if (iPSDataQuery.getName().equals("VIEW"))
            return projecttaskestimateService.selectView(searchContext);
        return null;
    }
        ...
        if (iPSDEAction != null) {
            if (iPSDEAction.getName().equals("Create")) {
                ...
                if(args[0] instanceof ProjectTaskestimate){
                    ...
                    CachedBeanCopier.copy(projecttaskestimateService.get(arg.getId()), arg);
                    return arg;
                }else{
            ...
            if (strActionName.equalsIgnoreCase("Create")) {
                ...
                if(args[0] instanceof ProjectTaskestimate){
                    ProjectTaskestimate arg = (ProjectTaskestimate) args[0] ;
                    CachedBeanCopier.copy(projecttaskestimateService.sysGet(arg.getId()), arg);
                    return arg;
                }else{
                    ...
                    return projecttaskestimateService.sysUpdate((ProjectTaskestimate) args[0]);
            }             
        }
