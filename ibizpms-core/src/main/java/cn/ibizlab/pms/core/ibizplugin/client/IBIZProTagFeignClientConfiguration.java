...
public class IBIZProTagFeignClientConfiguration {
    ...
    public IBIZProTagFeignClient iBIZProTagFeignClient(Decoder decoder, Encoder encoder, Client client, Contract contract, List<RequestInterceptor> requestInterceptors) {
        ...
        if (builder instanceof HystrixFeign.Builder) {
            ...
            return nameBuilder.target(IBIZProTagFeignClient.class, "http://" + serviceValue, new IBIZProTagFallback());
        } else {
            Feign.Builder nameBuilder = Feign.builder()
                    ...
                    .contract(contract)
                    .requestInterceptors(requestInterceptors);
            return nameBuilder.target(IBIZProTagFeignClient.class, "http://" + serviceValue);
        }
    }
}
