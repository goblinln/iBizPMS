...
public class WebHookModelImpl extends DataEntityModelImpl {
    ...
    public boolean test(Serializable key, String action) {
        ...
        List<WebHook> domains = webhookService.searchDefault(context).getContent();
        if (domains.size() == 0) {
            return false;
        }

        try {
            return SpringContextHolder.getBean(cn.ibizlab.pms.core.zentao.runtime.WebHookRuntime.class).testDataAccessAction(domains.get(0),action);
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean test(List<Serializable> keys, String action) {
        ...
        try {
            for(WebHook domain : domains){
                if(SpringContextHolder.getBean(cn.ibizlab.pms.core.zentao.runtime.WebHookRuntime.class).testDataAccessAction(domain,action)){
                    return false;
                }
            }
        } catch (Exception e) {
