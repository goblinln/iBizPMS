...
public class IbizproProductMonthlyRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime {
    ...
    public List<IbizproProductMonthly> selectDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        ...
        if (iPSDataQuery.getName().equals("DEFAULT"))
            return ibizproproductmonthlyService.selectQueryByDefault(searchContext);
        if (iPSDataQuery.getName().equals("VIEW"))
            return ibizproproductmonthlyService.selectQueryByView(searchContext);
        return null;
    }
    ...
    public IbizproProductMonthly selectOne(ISearchContextBase iSearchContextBase) {
        ...
        List<IbizproProductMonthly> domains = ibizproproductmonthlyService.select(searchContext);
        if (domains.size() == 0)
            return null;
