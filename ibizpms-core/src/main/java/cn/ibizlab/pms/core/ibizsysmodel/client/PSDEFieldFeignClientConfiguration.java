...
/**
 * 实体[PSDEField] 服务对象接口配置
 */
@Configuration
public class PSDEFieldFeignClientConfiguration {
    ...
    public PSDEFieldFeignClient pSDEFieldFeignClient(Decoder decoder, Encoder encoder, Client client, Contract contract, List<RequestInterceptor> requestInterceptors) {
        ...
        if (builder instanceof HystrixFeign.Builder) {
            ...
            return nameBuilder.target(PSDEFieldFeignClient.class, "http://" + serviceValue, new PSDEFieldFallback());
        } else {
            Feign.Builder nameBuilder = Feign.builder()
                    ...
                    .contract(contract)
                    .requestInterceptors(requestInterceptors);
            return nameBuilder.target(PSDEFieldFeignClient.class, "http://" + serviceValue);
        }
    }
}
