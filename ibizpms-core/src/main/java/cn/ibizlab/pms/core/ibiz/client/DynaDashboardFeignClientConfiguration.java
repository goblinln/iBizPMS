...
public class DynaDashboardFeignClientConfiguration {
    ...
    public DynaDashboardFeignClient dynaDashboardFeignClient(Decoder decoder, Encoder encoder, Client client, Contract contract, List<RequestInterceptor> requestInterceptors) {
        ...
        if (builder instanceof HystrixFeign.Builder) {
            ...
            return nameBuilder.target(DynaDashboardFeignClient.class, "http://" + serviceValue, new DynaDashboardFallback());
        } else {
            Feign.Builder nameBuilder = Feign.builder()
                    ...
                    .contract(contract)
                    .requestInterceptors(requestInterceptors);
            return nameBuilder.target(DynaDashboardFeignClient.class, "http://" + serviceValue);
        }
    }
}
