...
@Component("IBZTestReportActionRuntime")
@Slf4j
public class IBZTestReportActionRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime {

    @Autowired
    IIBZTestReportActionService ibztestreportactionService;
    ...
    protected IEntityBase getSimpleEntity(Object o) {
        if (o instanceof net.ibizsys.runtime.util.IEntityBase) {
            ...
            return ibztestreportactionService.sysGet((Long) o);
        }
    }

    @Override
    public String getId() {
        return "PSMODULES/ibiz/PSDATAENTITIES/IBZTestReportAction.json";
    }

    @Override
    public String getName() {
        return "IBZPRO_TESTREPROTACTION";
    }

    @Override
    public IEntityBase createEntity() {
        return new IBZTestReportAction();
    }

    @Override
    protected IService getService() {
        return this.ibztestreportactionService;
    }

    @Override
    public IBZTestReportActionSearchContext createSearchContext() {
    ...
    public IEntityBase deserializeEntity(Object objData) {
        try {
            return MAPPER.readValue(String.valueOf(objData),IBZTestReportAction.class);
        } catch (Exception e) {
    ...
    @Override
    public Page<IBZTestReportAction> searchDataSet(IPSDEDataSet iPSDEDataSet, ISearchContextBase iSearchContextBase) {
        IBZTestReportActionSearchContext searchContext = (IBZTestReportActionSearchContext) iSearchContextBase;
        if (iPSDEDataSet.getName().equals("DEFAULT"))
            return ibztestreportactionService.searchDefault(searchContext);    
        if (iPSDEDataSet.getName().equals("Type"))
            return ibztestreportactionService.searchType(searchContext);    
        return null;
    }

    @Override
    public List<IBZTestReportAction> selectDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        IBZTestReportActionSearchContext searchContext = (IBZTestReportActionSearchContext) iSearchContextBase;
        if (iPSDataQuery.getName().equals("DEFAULT"))
            return ibztestreportactionService.selectDefault(searchContext);
        if (iPSDataQuery.getName().equals("SIMPLE"))
            return ibztestreportactionService.selectSimple(searchContext);
        if (iPSDataQuery.getName().equals("Type"))
            return ibztestreportactionService.selectType(searchContext);
        if (iPSDataQuery.getName().equals("VIEW"))
            return ibztestreportactionService.selectView(searchContext);
        return null;
    }

    @Override
    public List<IBZTestReportAction> select(ISearchContextBase iSearchContextBase) {
        IBZTestReportActionSearchContext searchContext = (IBZTestReportActionSearchContext) iSearchContextBase;
        return ibztestreportactionService.select(searchContext);
    }
    ...
    protected void onExecuteActionLogics(Object arg0, IPSDEAction iPSDEAction, String strAttachMode, IDynaInstRuntime iDynaInstRuntime, Object joinPoint) throws Throwable {
        IBZTestReportAction entity = null;
        String action = iPSDEAction.getName();
        if (arg0 instanceof IBZTestReportAction) {
            entity = (IBZTestReportAction) arg0;
        } else {
            if (arg0 != null && "get".equalsIgnoreCase(action)) {
                entity = ibztestreportactionService.sysGet((Long) arg0);
            } else if ("remove".equalsIgnoreCase(action)) {
                entity = new IBZTestReportAction();
    ...
    @Around("execution(* cn.ibizlab.pms.core.ibiz.service.impl.IBZTestReportActionServiceImpl.*(..))")
    @Transactional
