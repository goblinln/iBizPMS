...
@Slf4j
public class TaskStatsRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime {
    ...
    @Override
    public Page<TaskStats> searchDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        //暂未实现
        return null;
    }
    ...
    public TaskStats selectOne(ISearchContextBase iSearchContextBase) {
        ...
        List<TaskStats> domains = taskstatsService.select(searchContext);
        if (domains.size() == 0)
            return null;
        ...
        if (iPSDEAction != null) {
            if (iPSDEAction.getName().equals("Create")) {
                ...
                if(args[0] instanceof TaskStats){
                    ...
                    arg = taskstatsService.get(arg.getId()) ;
                    return arg;
                }else{
