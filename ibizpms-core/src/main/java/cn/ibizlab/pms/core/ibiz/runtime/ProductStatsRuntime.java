...
public class ProductStatsRuntime extends cn.ibizlab.pms.core.runtime.SystemDataEntityRuntime {
    ...
    @Override
    public Page<ProductStats> searchDataQuery(IPSDEDataQuery iPSDataQuery, ISearchContextBase iSearchContextBase) {
        //暂未实现
        return null;
    }
    ...
    public ProductStats selectOne(ISearchContextBase iSearchContextBase) {
        ...
        List<ProductStats> domains = productstatsService.select(searchContext);
        if (domains.size() == 0)
            return null;
        ...
        if (iPSDEAction != null) {
            if (iPSDEAction.getName().equals("Create")) {
                ...
                if(args[0] instanceof ProductStats){
                    ...
                    arg = productstatsService.get(arg.getId()) ;
                    return arg;
                }else{
