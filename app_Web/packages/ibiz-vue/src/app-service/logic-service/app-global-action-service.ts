...
export class AppGlobalService {
    ...
    public SaveAndStart(args: any[], contextJO?: any, params?: any, $event?: any, xData?: any, actionContext?: any, srfParentDeName?: string) {
        ...
        const openStartView: Function = (item: any, localdata: any) => {
            if (item['wfversion']) {
                if (_this.viewInstance.getPSAppViewRefs && _this.viewInstance.getPSAppViewRefs.length > 0) {
                    let targetView: any = _this.viewInstance.getPSAppViewRefs.find((element: any) => {
                        return `WFSTART@${item['wfversion']}` === element.name;
                    })
                    let targetOpenView: any = targetView.getRefPSAppView;
                    if (targetOpenView && targetOpenView.modelref && targetOpenView.path) {
                        DynamicService.getInstance(_this.context).getAppViewModelJsonData(targetOpenView.path).then((result: any) => {
                            // 准备参数
                            ...
                            Object.assign(tempContext, { viewpath: result.dynaModelFilePath });
                            const appmodal = _this.$appmodal.openModal({ viewname: 'app-view-shell', title: result.title, height: result.height, width: result.width }, tempContext, tempViewParam);
