import { DynamicService, ViewTool, Util } from "ibiz-core";
import { IBizAppFuncModel } from 'ibiz-core';
...
export class AppFuncService {
     ...
     * @memberof AppFuncService
     */
    public async getViewModeJsonData(appView: any, context: any = {}) {
        let result: any = {};
        if (appView.modelref) {
            if (appView.path) {
                result = await DynamicService.getInstance(context).getAppViewModelJsonData(appView.path);
            } else {
                result = await DynamicService.getInstance(context).getAppViewModelJsonData(appView.id);
            }
        }
        return result;
    }
     ...
     * @memberof AppFuncService
     */
    public async executeApplication(appFunc: IBizAppFuncModel, context: any) {
        if (appFunc) {
            let viewParam: any = {};
     ...
     * @memberof AppFuncService
     */
    public async openAppView(appFunc: IBizAppFuncModel, context: any, viewparam: any = {}) {
        const appView = await this.getViewModeJsonData(appFunc.getPSAppView,context);
    ...
    public async processingParameter(context: any, appView: any, deResParameters: any[], parameters: any[]) {
        let params = [];
        if (appView.getPSAppDataEntity && appView.getPSAppDataEntity.modelref) {
            let result: any = {};
            if (appView.getPSAppDataEntity.path) {
                result = await DynamicService.getInstance(context).getAppEntityModelJsonData(appView.getPSAppDataEntity.path);
            }
     ...
     * @memberof AppFuncService
     */
    public openHtmlPage(appFunc: IBizAppFuncModel, context: any, viewparam: any) {
     ...
     * @memberof AppFuncService
     */
    public openPdAppFunc(appFunc: IBizAppFuncModel, context: any, viewparam: any) {
     ...
     * @memberof AppFuncService
     */
    public executeJavaScript(appFunc: IBizAppFuncModel, context: any, viewparam: any) {
     ...
     * @memberof AppFuncService
     */
    public custom(appFunc: IBizAppFuncModel, context: any, viewparam: any) {
