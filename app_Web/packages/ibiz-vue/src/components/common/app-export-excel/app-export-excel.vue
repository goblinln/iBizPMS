<template>
    <dropdown v-if="itemLevel === 0" :transfer="true" trigger='click'>
        <i-button :disabled="item.disabled" :loading="loading">
    ...
    public exportExcel($event: any, type: string): void {
        ...
        if (Object.is(type, 'maxRowCount')) {
            ...
            if (!this.startPage || !this.endPage) {
                this.$warning((this.$t('components.appExportExcel.desc') as string));
                return;
            }
            ...
            if (Number.isNaN(startPage) || Number.isNaN(endPage)) {
                this.$warning((this.$t('components.appExportExcel.desc1') as string));
                return;
            }
