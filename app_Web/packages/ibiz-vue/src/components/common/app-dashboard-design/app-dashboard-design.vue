            ...
            <el-menu v-show="!filterVal" :unique-opened="true">
                <template v-for="(item, index) of list">
                    <el-submenu :key="item.type + index" :index="item.type + index">
                        <div slot="title">{{Object.is(item.type, 'app') ? '全局' : item.name}}</div>
...
export default class AppDashboardDesign extends Vue {
     ...
     */
    @Prop() protected customModel: any;
    ...
    public async loadList() {
		let result = await this.designService.loadPortletList(this.context, this.viewparams);
    ...
    public notifyState(){
        setTimeout(() => {
          if (this.viewState) {
            this.layoutModel?.forEach((item: any) => {
              this.viewState.next({
                tag: item.modelData.name,
                action: "load",
    ...
    getPortletInstance(layoutModel: any){
        return this.portlets.find((item: any)=>{ return layoutModel.i === item.portletCodeName})?.modelData;
    }
