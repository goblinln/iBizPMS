...
import { IBizFormDetailContainerModel } from 'ibiz-core/src/model/control/ibiz-form-model/ibiz-form-detail-model/ibiz-form-detail-container-model';
import { IBizFormDetailModel } from 'ibiz-core/src/model/control/ibiz-form-model/ibiz-form-detail-model/ibiz-form-detail-model';
...
export class AppFormBase extends EditFormControlBase {
    ...
    public renderDruipart(modelJson: any, index: number): any {
        const { getPSSysCss, refreshItems, getLayoutPos, height, appView, parentdata, localContext, localParam, appDERSPaths } = modelJson;
        const { appDataEntity } = this.controlInstance;
        const { dynaModelFilePath } = appView;
        let tempContext: any = Util.deepCopy(this.context);
        Object.assign(tempContext, { viewpath: dynaModelFilePath });
        // druipart样式
    ...
    public renderFormItem(modelJson: IBizFormItemModel, index: number): any {
        const { editor, compositeItem, formItems, contentWidth, contentHeight} = modelJson;
        const refFormItem: any = [];
        const style = {
            width: contentWidth ? contentWidth : false,
            height: contentHeight ? contentHeight: false
        }
        if(compositeItem && formItems){
