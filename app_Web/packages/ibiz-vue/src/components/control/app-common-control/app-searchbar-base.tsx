...
import { SearchBarControlBase } from '../../../widgets/SearchBarControlBase';
...
export class AppSearchBarBase extends SearchBarControlBase {
    ...
    public renderBarFilters(data: any) {
        ...
        if(!barFilters || barFilters.length == 0 || !data.editor) {
            return null;
        }
        return barFilters.map((filter: any) => {
            if(!filter.name || !Object.is(data.editor, filter.name)) {
                return null;
            }
            const editor = new IBizEditorModel(filter?.getPSEditor, this.context);
            if(editor) {
