...
export class AppPortletBase extends PortletControlBase {
    ...
    public renderControl() {
        if (this.controlInstance.getAllControls()?.length > 0) {
            // 绘制其他部件
            let newStaticProps = { ...this.staticProps };
            let newDynamicProps = { ...this.dynamicProps };
            newStaticProps.modelData = this.controlInstance.getAllControls()[0];
            return this.$createElement('app-control-shell', {
                props: { staticProps: newStaticProps, dynamicProps: newDynamicProps },
                ref: this.controlInstance.controls[0].name ? this.controlInstance.controls[0].name : this.controlInstance.controls[0].codeName,
                on: {
                    'ctrl-event': (props: any) => {
                        this.ctrlEvent(props);
                    },
                    closeView: ($event: any) => {
    ...
    public renderRawItem() {
        const { getPSSysCss: sysCss, rawItemHeight, rawItemWidth, contentType, rawContent, htmlContent, sysImage } = this.controlInstance;
     ...
     */
    public renderCustom() {
        let { render } = this.controlInstance;
        // todo自定义绘制
        if (render) {
            return render?.code;
        } else {
    ...
    public renderAppMenu() {
        let menuInstance = this.controlInstance.getAllControls()?.[0];
        if (menuInstance) {
     ...
     */
    public renderView() {
        if (!this.controlInstance.portletAppView) {
            return;
        }
        const { dynaModelFilePath, name } = this.controlInstance.portletAppView;
        ...
        return this.$createElement('app-view-shell', {
            props: {
                staticProps: {
                    ...
                    viewModelData: this.controlInstance.portletAppView
                },
                dynamicProps: {
     ...
     */
    public renderTitle() {
        const { getPSSysImage: image, portletType } = this.controlInstance;
        return [
    ...
    public renderUiAction() {
        const { actionGroupDetails } = this.controlInstance;
        if (!actionGroupDetails) {
            return
        }
        return <span class="portlet-action">
            {actionGroupDetails.map((actionDetail: any) => {
                const { getPSUIAction: uiAction, showCaption } = actionDetail;
                let uiactionName = uiAction?.codeName?.toLowerCase() || actionDetail.name?.toLowerCase();
                // 显示内容
                let contentElement = <a on-click={(e: any) => { this.handleActionClick(e, actionDetail) }} v-show="uiactionModel['${uiactionname}'].visabled" >
    ...
    public handleActionClick(event: any, detail: any) {
        const { name, getPSAppViewLogics } = this.controlInstance;
        let data = Util.deepCopy(this.context);
        AppViewLogicService.getInstance().excuteViewLogic(`${name}_${detail.name}_click`, event, this, data, getPSAppViewLogics);
    }
