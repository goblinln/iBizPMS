...
export class AppTabExpPanelBase extends TabExpPanelBase {
    ...
    public renderTabPaneContent(tabViewPanel: any) {
        ...
        if (tabViewPanel?.getPSSysCounterRef?.id && tabViewPanel?.counterId) {
            const targetCounterService: any = Util.findElementByField(this.counterServiceArray, 'id', tabViewPanel?.getPSSysCounterRef?.id)?.service;
            viewPanelCount = targetCounterService?.counterData?.[tabViewPanel.counterId]
        }
