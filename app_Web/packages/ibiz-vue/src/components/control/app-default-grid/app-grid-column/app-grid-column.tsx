...
export class AppDefaultGridColumn extends Vue {
    ...
    public renderGridColumn() {
        const { codeList, cLConvertMode, enableLinkView, $linkView, linkViewEntity, name } = this.columnInstance;
        if (codeList && cLConvertMode == "FRONT") {   //  代码表
            ...
            this.handleLinkViewParams($linkView, view, linkViewEntity);
     ...
     * @param entity 链接视图实体
     */
    public handleLinkViewParams(linkView: any, view: any, entity?: any) {
        //获取父关系路由参数
        let tempDeResParameters: any[] = linkView.getPSAppDERSPaths ? Util.formatAppDERSPath(this.context, linkView.getPSAppDERSPaths) : [];
        //视图本身路由参数
    ...
    public renderLinkView(view: any) {
        const { linkViewEntity: entity } = this.columnInstance;
        ...
        return (
            <app-column-link
                deKeyField={entity ? entity.codeName?.toLowerCase() : ""}
                context={tempContext}
