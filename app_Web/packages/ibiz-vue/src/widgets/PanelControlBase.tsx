import { ViewTool, Verify, Util, IBizEditorModel } from 'ibiz-core';
import { MDControlBase } from "./MDControlBase";
import { AppPanelModel } from "../ctrl-model";
...
export class PanelControlBase extends MDControlBase {
    ...
    public async ctrlModelInit(args?: any) {
        ...
        if (this.controlInstance.getRootPSPanelItems?.length > 0) {
     ...
     * @memberof PanelControlBase
     */
    public initRules(panelItems: any[]) {
        // 初始化非空值规则和数据类型值规则
        panelItems.forEach((panelItem: any) => {
            if (panelItem?.getPSPanelItems?.length > 0) {
                this.initRules(panelItem.getPSPanelItems);
            } else {
                if (panelItem.getPSEditor) {
                    let editorRules: any = Verify.buildVerConditions(new IBizEditorModel(panelItem.getPSEditor, this.context));
    ...
    public buttonClick(controlName: string, data: any, $event: any){
        AppViewLogicService.getInstance().excuteViewLogic(`${controlName?.toLowerCase()}_${data.tag}_click`, $event, this, data, this.controlInstance.getPSAppViewLogics);
    }
    ...
    public verifyGroupLogic(data: any, logic: any){
        if(logic.logicType == 'GROUP' && logic.getPSPanelItemLogics?.length > 0){
            let result: boolean = true;
            if(logic.groupOP == 'AND'){
                let falseItem = logic.getPSPanelItemLogics.find(( childLogic: any)=>{
                    return !this.verifyGroupLogic(data, childLogic);
                })
                result = falseItem == undefined;
            }else if(logic.groupOP == 'OR'){
                let trueItem = logic.getPSPanelItemLogics.find(( childLogic: any)=>{
                    return this.verifyGroupLogic(data, childLogic);
                })
            ...
            return Verify.testCond(data[logic.dstModelField.toLowerCase()], logic.condOp, logic.value)
        }
        return false;
    }
     ...
     * @memberof PanelControlBase
     */
    public initItemsActionModel(panelItems: any[]){
        panelItems.forEach((panelItem: any) => {
            if (panelItem?.getPSPanelItems?.length > 0) {
                this.initItemsActionModel(panelItem.getPSPanelItems);
            }else if(panelItem?.itemType == 'BUTTON' && panelItem.getPSUIAction){
                const appUIAction: any = Util.deepCopy(panelItem.getPSUIAction);
    ...
    public onStaticPropsChange(newVal: any, oldVal: any) {
        ...
        this.dataModel = new AppPanelModel(newVal.modelData);
        ...
        this.initDetailsModel(getRootPSPanelItems);
    }
     ...
     * @memberof PanelControlBase
     */
    public initDetailsModel(panelItems: any[]) {
        if (panelItems?.length > 0) {
            panelItems.forEach((panelItem: any) => {
                ...
                switch (panelItem.itemType) {
                    case 'BUTTON':
                        Object.assign(detailModel,{
                            uiaction: {
                                type: panelItem.getPSUIAction?.uIActionType, 
                                tag: panelItem.getPSUIAction?.uIActionTag,
                                actiontarget: panelItem.getPSUIAction?.actionTarget,
                                noprivdisplaymode: panelItem.getPSUIAction?.uIActionMode,
                                dataaccaction: panelItem.getPSUIAction?.dataAccessAction,
                                visible: true,
                                disabled: false
                            }
                        });
                ...
                if (Object.is(panelItem.itemType,'CONTAINER')) {
                    this.initDetailsModel(panelItem.getPSPanelItems);
                }
