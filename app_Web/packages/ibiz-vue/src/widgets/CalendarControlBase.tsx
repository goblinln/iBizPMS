...
import { Util, ViewTool, IBizCalendarModel } from 'ibiz-core';
...
export class CalendarControlBase extends MDControlBase{
    ...
    public initActionModel() {
        ...
        if(calendarItems?.length>0) {
            calendarItems.forEach((item: any) => {
                if(item.getPSDEContextMenu && item.getPSDEContextMenu.getPSDEToolbarItems?.length>0) {
                    let toobarItems: any[] = item.getPSDEContextMenu.getPSDEToolbarItems;
                    toobarItems.forEach((toolbarItem: any) => {
     ...
     * @memberof CalendarControlBase
     */
    public initActionModelItem(toolbarItem: any,item: any,tempModel: any){
        let tempItem: any = {
            name: toolbarItem.name,
            nodeOwner: item.itemType
        }
        if(toolbarItem.getPSUIAction) {
            let uiAction = toolbarItem.getPSUIAction
            tempItem.type = item.itemType;
            tempItem.tag = uiAction.id;
            tempItem.visabled = true;
            tempItem.disabled = false;
            if(uiAction?.getActionTarget && uiAction?.getActionTarget != ""){
                tempItem.actiontarget = uiAction.getActionTarget
            }
            if(uiAction.getNoPrivDisplayMode) {
                tempItem.noprivdisplaymode = uiAction.getNoPrivDisplayMode;
            }
    ...
    public initShowLegend() {
        ...
        if(calendarItems?.length>0) {
            calendarItems.forEach((item: any) => {
                this.isShowlegend[item.itemType] = true;
            })
        }
    }
    ...
    public onEventClick($event: any, isOriginData:boolean = false, $event2?: any) {
        ...
        const item = this.controlInstance.calendarItems?.find((_item: any) => { return _item.itemType == event.itemType });
        if(item) {
            const codeName = item.getPSAppDataEntity?.codeName.toLowerCase();
            if(codeName) {
    ...
    public onEventDrop($event: any) {
        ...
        const item = this.controlInstance.calendarItems?.find((_item: any) => { return _item.itemType == itemType });
        if(item) {
            const codeName = item.getPSAppDataEntity?.codeName.toLowerCase();
            if(codeName) {
    ...
    public ctrlInit() {
        super.ctrlInit();
        if(this.calendarType && this.calendarType == 'TIMELINE'){
            this.searchEvents();
        } else {
            this.setButtonText();
        }
    ...
    public eventRender(info?:any,) {
        ...
        info.el.addEventListener('contextmenu', (event: MouseEvent) => {
            ...
            let dataMapping:any = {};
            if( this.controlInstance.calendarItems?.length > 0 ) {
                dataMapping = this.controlInstance.calendarItems.find((calendarItem: any) => {
                    return calendarItem.itemType == data.itemType;
                })
            }
            if(!dataMapping.getPSAppDataEntity.codeName){
                return;
            }
            this.computeNodeState(data,dataMapping.getPSAppDataEntity.codeName).then((result:any) => {
    ...
    public renderContextMenu(event: any) {
        ...
        const { calendarItems } = this.controlInstance;
        if(calendarItems?.length>0) {
            let item: any = calendarItems.find((_item: any) => {
                return _item.itemType == event.itemType;
            })
            if(item && item.getPSDEContextMenu && item.getPSDEContextMenu.controlType == "CONTEXTMENU") {
                let contextMenu = this.controlInstance.getControl(item.getPSDEContextMenu.name);
                if(contextMenu){
    ...
    public itemClick(tag: any, $event: any) {
        AppViewLogicService.getInstance().excuteViewLogic(`calendar_quicktoolbar_${tag}_click`, $event, this, {}, this.controlInstance.getPSAppViewLogics);
    }
    ...
    public initQuickToolbar() {
        const { quickToolbar } = this.controlInstance;
        if(quickToolbar && quickToolbar.getPSDEToolbarItems?.length>0) {
            let items: Array<any> = quickToolbar.getPSDEToolbarItems.filter((item: any) => {
    ...
    public initQuickToolbarItemModel() {
        const items: Array<any> = this.quickToolbarItems;
        let models: any = {};
        if(items.length>0) {
            items.forEach((item: any) => {
                ...
                Object.assign(model, {
                    ...
                    uiaction: {
                        tag: uiAction.uIActionTag ? uiAction.uIActionTag : uiAction.id,
                        target: uiAction?.actionTarget
                    }
                });
                Object.assign(models, { [`${name.toLowerCase()}`]: model });
            })
        }
    ...
    public onCtrlEvent(controlname: string, action: string, data: any) {
        if(action == 'contextMenuItemClick'){
            AppViewLogicService.getInstance().excuteViewLogic(`${controlname}_${data}_click`, undefined, this, {}, this.controlInstance.getPSAppViewLogics);
        }else{
