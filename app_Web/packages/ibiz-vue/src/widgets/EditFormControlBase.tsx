...
export class EditFormControlBase extends FormControlBase {
    ...
    */
    ...
    public onFormItemActionClick({ tag, event }: any) {
        AppViewLogicService.getInstance().excuteViewLogic(`${this.controlInstance.name}_${tag}_click`, event, this, tag, this.controlInstance.getPSAppViewLogics);
    }
    ...
    public updateDefault() {
        ...
        if (allFormDetails.length > 0) {
            for (const detail of allFormDetails) {
                const property = detail.codeName.toLowerCase();
                if (detail.updateDV && this.data.hasOwnProperty(property)) {
            ...
            for (const detail of allFormDetails) {
                if (detail.detailType == 'FORMITEM' && detail.editorType != 'HIDDEN' && !detail.compositeItem) {
                    ...
                    this.rules[detail.name] = [
                        // 非空值规则
                        { validator: (rule: any, value: any, callback: any) => { return !(this.detailsModel[detail.name].required && !value) }, message: `${detail.caption} 必须填写` },
                        // 表单值规则
                        ...otherRules,
                        // 编辑器基础值规则
                        ...editorRules
    ...
    public handleActionClick(event: any, formDetail: any, actionDetal: any) {
        AppViewLogicService.getInstance().excuteViewLogic(this.getViewLogicTag('form', formDetail.codeName, actionDetal.name), event, this, undefined, this.controlInstance.getPSAppViewLogics);
    }
}
