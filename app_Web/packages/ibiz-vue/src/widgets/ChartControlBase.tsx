...
import { Util, IBizChartModel, ChartLineSeries, ChartFunnelSeries, ChartPieSeries, ChartBarSeries, ChartRadarSeries } from 'ibiz-core';
...
export class ChartControlBase extends MDControlBase {
    ...
    public async initSeriesModel() {
        if (!this.controlInstance.controlModelData?.getPSDEChartSerieses?.length) {
            return
        }
        for (let index = 0; index < this.controlInstance.controlModelData?.getPSDEChartSerieses?.length; index++) {
            const series: any = this.controlInstance.controlModelData?.getPSDEChartSerieses[index];
    ...
    public getSeriesModelParam(series: any): Promise<any> {
        ...
        const opts: any = {
            ...
            data: [],
            seriesMap: {},
            categorCodeList: { type: series?.getCatalogPSCodeList?.codeListType, tag: series?.getCatalogPSCodeList?.codeName, emptycode: 'empty', emptytext: series?.getCatalogPSCodeList?.getEmptyText },
            ...
            ecxObject: {
                ...
                emphasis: {
                    label: {
                        // zktodo 缺少 item.containsUserParam("ECX.emphasis")
                        show: true,
                        fontSize: 20
                    }
                }
            },
            seriesCodeList: series?.seriesCodeList,
    ...
    public fillUserParam(param: any, opts: any, tag: string) {
        if (!param.getUserParam) {
            return
        }
        switch (tag) {
            case 'ECX':
                if (param.getUserParam['ECX.label']) {
                    opts['label'] = param.getUserParam['ECX.label'];
                }
                if (param.getUserParam['ECX.labelLine']) {
                    opts['labelLine'] = param.getUserParam['ECX.labelLine'];
                }
                if (param.getUserParam['ECX.itemStyle']) {
                    opts['itemStyle'] = param.getUserParam['ECX.itemStyle'];
                }
                if (param.getUserParam['ECX.emphasis']) {
                    opts['emphasis'] = param.getUserParam['ECX.emphasis'];
                }
                for (const key in param.getUserParam) {
                    if (Object.prototype.hasOwnProperty.call(param.getUserParam, key)) {
                        if (key.indexOf('EC.') != -1) {
                            const value = param.getUserParam[key].trim();
            ...
            case 'EC':
                for (const key in param.getUserParam) {
                    if (Object.prototype.hasOwnProperty.call(param.getUserParam, key)) {
                        if (key.indexOf('EC.') != -1) {
                            const value = param.getUserParam[key].trim();
                            opts[key.replace('EC.', "")] = this.isJson(value) ? JSON.parse(value) : this.isArray(value) ? eval(value) : value;
                        }
                    }
                }
                break;
    ...
    public initChartUserParams() {
        this.fillUserParam(this.controlInstance.controlModelData, this.chartUserParams, 'EC')
    }
    ...
    public initChartBaseOPtion() {
        this.chartBaseOPtion = (new Function("return {" + this.controlInstance.controlModelData.baseOptionJOString + '}'))()
    }
    ...
    public initChartOption() {
        ...
        // 填充series
        const indicator: any = [];
        this.controlInstance?.controlModelData?.getPSDEChartSerieses?.forEach((_series: any) => {
            series.push(this.fillSeries(_series, indicator));

        });
        // 填充xAxis
        const xAxis: any = [];
        this.controlInstance?.controlModelData?.getPSChartXAxises?.forEach((_xAxis: any) => {
            xAxis.push(this.fillAxis(_xAxis));
        });
        // 填充yAxis
        const yAxis: any = [];
        this.controlInstance?.controlModelData?.getPSChartYAxises?.forEach((_yAxis: any) => {
            yAxis.push(this.fillAxis(_yAxis));

        });
        // 填充grid
        const grid: any = [];
        this.controlInstance?.controlModelData?.getPSChartGrids?.forEach((_grid: any) => {
            grid.push({
        ...
        const opt = {
            ...
            dataset: [],
            legend: {
                show: this.controlInstance.chartLegend?.showLegend ? this.controlInstance?.chartLegend?.showLegend : false
            },
            series: series,
            xAxis: xAxis,
            yAxis: yAxis,
        ...
        if (this.controlInstance.title) {
            const _titleModel: any = this.controlInstance.title;
            const title = {
    ...
    public fillSeries(series: any, indicator: any = {}) {
        ...
        const assginCodeList = (item: any) => {
            item?.getPSCodeItems?.forEach((_item: any) => {
                ...
                _item.max = _item?.getUserParam?.MAXVALUE ? _item.getUserParam.MAXVALUE : 100;
                indicator.push(_item);
        ...
        switch (series.eChartsType) {
            case 'line':
            case 'bar':
                encode.x = this.arrayToLowerCase(series.getPSChartSeriesEncode.x)
                encode.y = this.arrayToLowerCase(series.getPSChartSeriesEncode.y)
                break;
            case 'pie':
            case 'funnel':
                encode.itemName = series?.getPSChartSeriesEncode?.category?.toLowerCase();
                encode.value = series?.getPSChartSeriesEncode?.value.toLowerCase();
                break;
            case 'radar':
        ...
        return {
            ...
            name: series.caption,
            type: series.eChartsType,
            xAxisIndex: series.xAxisIndex ? series.xAxisIndex : 0,
            yAxisIndex: series.yAxisIndex ? series.yAxisIndex : 0,
            datasetIndex: series.getPSChartDataSet.id ? series.getPSChartDataSet.id : 0,
            // zktodo 缺少json
            encode: encode,
    ...
    public getCodeList(codeListObject: any): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if (codeListObject.codeName && Object.is(codeListObject.codeListType, "STATIC")) {
                const codelist = this.$store.getters.getCodeList(codeListObject.codeName);
                if (codelist) {
                ...
                this.codeListService.getItems(codeListObject.codeName).then((res: any) => {
                    resolve(res);
                }).catch((error: any) => {
                    console.log(`----${codeListObject.codeName}----代码表不存在`);
                });
