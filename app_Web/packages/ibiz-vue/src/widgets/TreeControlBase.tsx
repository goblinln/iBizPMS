...
import { IBizTreeModel, Util, ViewTool } from 'ibiz-core';
...
export class TreeControlBase extends MDControlBase {
    ...
    public initActionModel() {
        const allTreeNodes = this.controlInstance.getAllTreeNodes();
        let tempModel: any = {};
        if(allTreeNodes?.length>0 ) {
            allTreeNodes.forEach((item: any) => {
    ...
    public onCtrlEvent(controlname: string, action: string, data: any) {
        if(action == 'contextMenuItemClick'){
            AppViewLogicService.getInstance().excuteViewLogic(`${controlname}_${data}_click`, undefined, this, undefined, this.controlInstance.getPSAppViewLogics);
        }else{
