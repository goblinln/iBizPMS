import { IBizKanbanModel, IBizToolBarItemModel, Util } from 'ibiz-core';
...
export class KanbanControlBase extends MDControlBase {
    ...
    public async ctrlModelInit(args?: any) {
        ...
        this.minorSortPSDEF = this.controlInstance?.minorSortPSDEF ? this.controlInstance.minorSortPSDEF : "";
        this.groupField = this.controlInstance?.groupField ? this.controlInstance.groupField.codeName?.toLowerCase() : "";
        this.updateGroupAction = this.controlInstance?.updateGroupAction;
        this.isGroup = this.controlInstance?.enableGroup;
        this.groupMode = this.controlInstance?.groupMode;
        this.groupCodelist = this.controlInstance?.codeList ? { type: this.controlInstance.codeList.codeListType, tag: this.controlInstance.codeList.codeName, data: this.controlInstance.codeList } : {};
    ...
    public initQuickAction() {
        ...
        if (this.controlInstance && this.controlInstance.controlToolBarItems) {
            this.controlInstance.controlToolBarItems.forEach((item: IBizToolBarItemModel) => {
    ...
    public handleItemClick(data: any, $event: any) {
        AppViewLogicService.getInstance().executeViewLogic(this.getViewLogicTag(this.controlInstance.name, 'quicktoolbar', data.tag), $event, this, undefined, this.controlInstance?.getPSAppViewLogics);
    }
    ...
    public uiActionClick(detail: any, $event: any, group: any) {
        ...
        AppViewLogicService.getInstance().executeViewLogic(this.getViewLogicTag(this.controlInstance.name, 'group', detail.name), $event, this, row, this.controlInstance?.getPSAppViewLogics);
    }
        ...
        post.then((response: any) => {
            ...
            if (!response || response.status !== 200) {
                if (response.errorMessage) {
                    this.$Notice.error({ title: "警告", desc: response.errorMessage });
                }
                return;
            }
    ...
    public updateData(opt: any) {
        ...
        if (this.controlInstance.getPSAppDataEntity) {
            Object.assign(_context, { [(this.controlInstance.getPSAppDataEntity as any).codeName.toLowerCase()]: opt.srfkey });
        }
