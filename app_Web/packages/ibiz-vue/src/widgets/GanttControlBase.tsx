...
import { Util, DynamicService, CodeListServiceBase } from 'ibiz-core';
...
export class GanttControlBase extends MDControlBase {
     ...
     * 初始化甘特图部件配置参数
     * 
     * @memberof AppDefaultGantt
     */
    public initOptions() {
        const { treeColumns } = this.controlInstance;
        if(treeColumns?.length>0) {
            let tempOptions: any[] = [];
            treeColumns.forEach((column: any) => {
                ...
                let option: any = {
                    ...
                    render: (task: any) => {
                        if(column.getRenderPSSysPFPlugin){
                            const pluginInstance:any = this.PluginFactory.getPluginInstance("CONTROLITEM",column.getRenderPSSysPFPlugin.pluginCode);
                            if(pluginInstance){
        ...
        for(let i = 0; i < treeNodes.length; i++){
            ...
            if(dataItems && dataItems.length > 0){
                for(let j = 0; j < dataItems.length; j++){
                    ...
                    if(dataItem.getFrontPSCodeList && dataItem.getFrontPSCodeList?.modelref && dataItem.getFrontPSCodeList?.path){
                        const codelistJson = await DynamicService.getInstance(this.context).getAppCodeListJsonData(dataItem.getFrontPSCodeList.path);
                        if(codelistJson){
                            let items = await this.codeListService.getDataItems({ type: codelistJson.codeListType, tag: codelistJson.codeName });
    ...
    public getColumnValue(task: any, field: string) {
        const { treeNodes } = this.controlInstance;
        const node: any = treeNodes?.find((_node: any) => {
            return task.id.split(";")[0] == _node.nodeType;
        });
        if(node?.getPSDETreeNodeDataItems?.length > 0) {
            const { getPSDETreeNodeDataItems: dataItems } = node;
            const dataItem = dataItems.find((_item: any) => {
