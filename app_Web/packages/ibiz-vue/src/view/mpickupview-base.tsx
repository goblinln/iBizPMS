import { MPickupViewEngine, IBizPickUpViewPanelModel, IBizMPickUpViewModel } from 'ibiz-core';
...
export class MPickUpViewBase extends MainViewBase {
     ...
     */
    public viewInstance!: IBizMPickUpViewModel;
    ...
    public async viewModelInit() {
        this.viewInstance = new IBizMPickUpViewModel(this.staticProps?.modeldata,this.context);
    ...
    public renderControlContent(){
      ...
      return this.$createElement(targetCtrlName, { props: targetCtrlParam, ref: this.viewInstance?.viewPickUpViewPanel?.name, on: targetCtrlEvent });
    }
            ...
            if(model.datas.length>0){
                model.datas.forEach((data:any,index:any)=>{
                    if(!data.srfmajortext){
                        let fieldCodeName = (this.viewInstance?.appDataEntity?.majorField?.codeName)?.toLowerCase();
