...
export class MDViewBase extends MainViewBase {
    ...
    public getQuickGroupDefaultSelect() {
        ...
        if (this.quickGroupCodeList?.getPSCodeItems?.length > 0) {
            for (const item of this.quickGroupCodeList.getPSCodeItems) {
                if (item.getPSCodeItems?.length > 0) {
                    defaultSelect = item.getPSCodeItems.find((_item: any) => { return _item.default; });
                }
