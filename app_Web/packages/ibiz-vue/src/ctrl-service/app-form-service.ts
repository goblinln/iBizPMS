import { ControlServiceBase, Util, IBizFormModel, IBizFormItemModel } from 'ibiz-core';
import { GlobalService } from 'ibiz-service';
import { AppFormModel, Errorlog } from 'ibiz-vue';
...
export class AppFormService extends ControlServiceBase {
    ...
    * @memberof MainModel
    */
    public controlInstance !: IBizFormModel;
    ...
    public add(action: string, context: any = {}, data: any = {}, isloading?: boolean, isWorkflow?: boolean): Promise<any> {
        ...
        this.controlInstance.formItems.find((item: IBizFormItemModel) => {
            if (!item.hidden && item.appDeField?.keyField) {
                Object.assign(Data, {
                    [this.controlInstance.appDataEntity.keyField.codeName.toLowerCase()]: data[item.name],
                    srffrontuf: '1'
                });
                return true;
            }
    ...
    public loadDraft(action: string, context: any = {}, data: any = {}, isloading?: boolean): Promise<any> {
        ...
        if (this.controlInstance.controlType != 'SEARCHFORM') {
            Data[this.controlInstance.appDataEntity.keyField.codeName.toLowerCase()] = PrimaryKey;
            Data[this.controlInstance.appDataEntity.codeName.toLowerCase()] = PrimaryKey;
        }
        return new Promise((resolve: any, reject: any) => {
            let result: Promise<any>;
            const _appEntityService: any = this.appEntityService;
            ...
            result.then((response) => {
                this.setRemoteCopyData(response);
                if (this.controlInstance.controlType != 'SEARCHFORM') {
                    response.data[this.controlInstance.appDataEntity.keyField.codeName.toLowerCase()] = PrimaryKey;
                }
                this.handleResponse(action, response, true);
                resolve(response);
            }).catch(response => {
