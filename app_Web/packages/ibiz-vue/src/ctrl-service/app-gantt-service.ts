import { ControlServiceBase, IBizGanttModel } from 'ibiz-core';
...
export class AppGanttService extends ControlServiceBase {
     ...
     */
    public controlInstance !: IBizGanttModel;
    ...
    public async initTreeNodeEntityService() {
        const { treeNodes } = this.controlInstance;
        if(treeNodes?.length>0) {
            for(const node of treeNodes) {
                const { $appDataEntity: appde, treeNodeType } = node;
                if(treeNodeType == "DE" && appde) {
    ...
    public async getNodes(context:any = {},data: any = {}, isloading?: boolean): Promise<any> {
        ...
        const node = treeNodes.find((_node: any) => {
     ...
     */
    @Errorlog
    public async fillNodes(node: any, args:{context: any, filter: any, list: any[], rsNavContext?: any, rsNavParams?: any, rsParams?: any}): Promise<any> {
        ...
        } else if(node.treeNodeType == "CODELIST" && node.getPSCodeList) {
            //TODO  代码表待测试
     ...
     * @memberof AppGanttService
     */
    public fillStaticNodes(node: any, args:{context: any, filter: any, list: any[], rsNavContext?: any, rsNavParams?: any, rsParams?: any}) {
        return new Promise((resolve:any,reject:any) => {
            ...
            } else if(getPSSysImage?.getImagePath) {
                Object.assign(treeNode, { icon: getPSSysImage.getImagePath })
            }
            Object.assign(treeNode, {
            ...
            //TODO  待补充rooTreeNode
            if(this.controlInstance.rootNode && Object.is(filter.srfnodeid, this.$treeNodeDelimiterMap.get(this.controlInstance.rootNode.nodeType.toUpperCase()))) {
     ...
     * @memberof AppGanttService
     */
    public async fillCodeListNodes(node: any, args:{context: any, filter: any, list: any[], rsNavContext?: any, rsNavParams?: any, rsParams?: any}) {
        const { context, filter, list } = args;
        const {
            getPSCodeList: codeList,
            nodeType,
            appendPNodeId,
            getPSSysImage,
            enableCheck,
                    ...
                    const handleChildData = (context: any, item: any) => {
                        ...
                        if(getPSSysImage) {
                            if(getPSSysImage.cssClass) {
                                ...
                                item["icon"] = getPSSysImage.getImagePath;
                            }
                        }
                        Object.assign(item, {
                            enablecheck: enableCheck ? true : false,
                    ...
                    codeListItems.forEach((item: any) => {
                        ...
                        if(getPSSysImage) {
                            if(getPSSysImage.cssClass) {
                                ...
                                treeNode["icon"] = getPSSysImage.getImagePath;
                            }
                        }
                        Object.assign(treeNode, {
                            enablecheck: enableCheck ? true : false,
                        ...
                        if(!Object.is(filter.srfnodeid, this.$treeNodeDelimiterMap.get(this.controlInstance.rootNode.nodeType.toUpperCase()))) {
                            Object.assign(treeNode, { parentId: filter.srfotherkey });
                        }
                        list.push(treeNode);
                        resolve(list);
     ...
     * @memberof AppGanttService
     */
    public fillDeNodes(node: any, args:{context: any, filter: any, list: any[], rsNavContext?: any, rsNavParams?: any, rsParams?: any}) {
        return new Promise((resolve:any,reject:any) => {
            const { context, filter, list } = args;
            const {
                $appDataEntity: appde,
                ...
                selected,
                appendPNodeId,
                getLeafFlagPSDEField
            } = node;
            let searchFilter: any = {};
            let nodeRSs: Array<any> = this.controlInstance.treeNodeRSs;
            if(nodeRSs?.length>0) {
                nodeRSs.forEach((noders: any) => {
                    if(noders.getChildPSDETreeNode?.id == id) {
                        ...
                        if(pickupfield && !Object.is(pickupfield, "")) {
                            if (Object.is(filter.strNodeType, this.$treeNodeDelimiterMap.get(noders.getParentPSDETreeNode?.id.toUpperCase()))) {
                ...
                this.searchNodeType(node, context, searchFilter, filter).then((records: any) => {
                    if(records?.length>0) {
                        records.forEach((entity: any) => {
                            let treeNode: any = {};
                            //整理Context
                            let strId: string = getIdPSAppDEField ? entity[getIdPSAppDEField.codeName.toLowerCase()] : entity[appde.keyField.codeName.toLowerCase()];
                            let strText: string = getTextPSAppDEField ? entity[getTextPSAppDEField.codeName.toLowerCase()] : entity[appde.getMajorPSAppDEField.codeName.toLowerCase()];
                            Object.assign(treeNode, {
                                srfparentdename: appde?.codeName,
                                srfparentkey: getIdPSAppDEField ? entity[getIdPSAppDEField.codeName.toLowerCase()] : entity[appde.keyField.codeName.toLowerCase()]
                            });
     ...
     * @memberof AppGanttService
     */
    public searchNodeType(node: any, context: any, searchFilter: any, filter: any) {
        return new Promise((resolve: any, reject: any) => {
            ...
            const {
                ...
                sortDir,
                $appDataEntity: appde,
                getPSAppDEDataSet: dataSet
            } = node;
            if(sortField && sortDir) {
                Object.assign(searchFilter,{sort: `${sortField},${sortDir}`});
            }
            if(appde && appde.codeName) {
                let _appEntityService: any = this.$nodeEntityServiceMap.get(appde.codeName);
                let list: any[] = [];
                if(_appEntityService && _appEntityService[dataSet.id] && _appEntityService[dataSet.id] instanceof Function) {
                    const response: Promise<any> = _appEntityService[dataSet.id](context, searchFilter, false);
     ...
     * @memberof AppGanttService
     */
    public async fillNodeChilds(node: any, context: any = {}, filter: any, list: any[]) {
        const { treeNodeRSs } = this.controlInstance;
        if(treeNodeRSs?.length>0) {
            let nodeRs: any = {};
            if(filter.srfnodefilter && !Object.is(filter.srfnodefilter, "")) {
                nodeRs = treeNodeRSs.find((_item: any) => {
            ...
            if(nodeRs) {
                ...
                let treeNode: any = this.controlInstance.treeNodes.find((_node: any) => {
