...
export class AppLogicFactory {
    ...
    * @memberof AppLogicFactory
    */
    public static async getInstance(modelData: any, context: any) {
        if (modelData.getPSSysPFPlugin) {
            const importPlugin: any = PluginService.getInstance().getPluginInstance("UIACTION", modelData.getPSSysPFPlugin.pluginCode);
            if (importPlugin) {
