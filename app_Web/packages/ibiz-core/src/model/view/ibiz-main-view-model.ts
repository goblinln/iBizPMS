...
export class IBizMainViewModel extends IBizViewModelBase {
    ...
    get viewToolBarItems() {
        let targetItems: any[] = [];
        const initToolBarItems = (item: any) => {
            if (item.getPSDEToolbarItems?.length > 0) {
                let items: Array<any> = [];
                item.getPSDEToolbarItems.forEach((_item: any) => {
                    items.push(initToolBarItems(_item));
                })
                Object.assign(item, {
                    items: items
                });
            }
            return new IBizToolBarItemModel(item);
        }
        if (this.viewToolBar && this.viewToolBar.getPSDEToolbarItems) {
            this.viewToolBar.getPSDEToolbarItems.forEach((element: any) => {
                targetItems.push(initToolBarItems(element));
            });
        }
        return targetItems;
    }
