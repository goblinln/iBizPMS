...
export class IBizAppMenuModel extends IBizControlBaseModel{
    ...
    public async loaded() {
        await super.loaded();
        let appModelData: any = await DynamicService.getInstance(this.context).getAppMpdeJsonData(AppServiceBase.getInstance().getAppEnvironment().appDynaModelFilePath);
