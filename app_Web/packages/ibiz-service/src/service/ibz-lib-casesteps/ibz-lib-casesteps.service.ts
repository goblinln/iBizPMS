...
export class IbzLibCasestepsService extends IbzLibCasestepsBaseService {
     ...
     * @memberof IbzLibCasestepsService
     */
    static getInstance(context?: any): IbzLibCasestepsService {
        const cacheKey: string = context?.srfdynainstid ? `${context.srfdynainstid}IbzLibCasestepsService` : `IbzLibCasestepsService`;
        if (!___ibz___.sc.has(cacheKey)) {
            new IbzLibCasestepsService({ context: context, tag: cacheKey });
        }
        return ___ibz___.sc.get(cacheKey);
    }
}
