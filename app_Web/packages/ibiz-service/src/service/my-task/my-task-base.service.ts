...
import { IMyTask, MyTask } from '../../entities';
import keys from '../../entities/my-task/my-task-keys';
import { isNil, isEmpty } from 'ramda';
import { PSDEDQCondEngine } from 'ibiz-core';
import { GetUserConcatLogic } from '../../logic/entity/my-task/get-user-concat/get-user-concat-logic';

/**
 * 任务服务对象基类
 *
 * @export
 * @class MyTaskBaseService
 * @extends {EntityBaseService}
 */
export class MyTaskBaseService extends EntityBaseService<IMyTask> {
    protected get keys(): string[] {
        return keys;
    }
    protected SYSTEMNAME = 'iBizPMS';
    protected APPNAME = 'Web';
    protected APPDENAME = 'MyTask';
    protected APPDENAMEPLURAL = 'MyTasks';
    ...
    newEntity(data: IMyTask): MyTask {
        return new MyTask(data);
    }

    async addLocal(context: IContext, entity: IMyTask): Promise<IMyTask | null> {
        return this.cache.add(context, new MyTask(entity) as any);
    }

    async createLocal(context: IContext, entity: IMyTask): Promise<IMyTask | null> {
        return super.createLocal(context, new MyTask(entity) as any);
    }

    async getLocal(context: IContext, srfKey: string): Promise<IMyTask> {
        const entity = this.cache.get(context, srfKey);
        return entity!;
    }

    async updateLocal(context: IContext, entity: IMyTask): Promise<IMyTask> {
        return super.updateLocal(context, new MyTask(entity) as any);
    }

    async getDraftLocal(_context: IContext, entity: IMyTask = {}): Promise<IMyTask> {
        return new MyTask(entity);
    }
