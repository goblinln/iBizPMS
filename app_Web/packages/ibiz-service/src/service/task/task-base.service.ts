...
export class TaskBaseService extends EntityBaseService<ITask> {
    ...
    async FetchMyFavorites(_context: any = {}, _data: any = {}): Promise<HttpResponse> {
        if (_context.account && _context.project && true) {
            return this.http.post(`/accounts/${_context.account}/projects/${_context.project}/tasks/fetchmyfavorites`, _data);
        }
        if (_context.project && true) {
            return this.http.post(`/projects/${_context.project}/tasks/fetchmyfavorites`, _data);
        }
        if (_context.account && true) {
            return this.http.post(`/accounts/${_context.account}/tasks/fetchmyfavorites`, _data);
        }
    return new HttpResponse(null, { status: 404, statusText: '无匹配请求地址!' });
    }
    /**
