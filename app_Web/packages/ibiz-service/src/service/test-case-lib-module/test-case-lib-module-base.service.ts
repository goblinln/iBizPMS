...
export class TestCaseLibModuleBaseService extends EntityBaseService<ITestCaseLibModule> {
    ...
    async Select(_context: any = {}, _data: any = {}): Promise<HttpResponse> {
        if (_context.testcaselib && _context.testcaselibmodule) {
            return this.http.get(`/testcaselibs/${_context.testcaselib}/testcaselibmodules/${_context.testcaselibmodule}/select`);
        }
    ...
    async FetchRoot_NoBranch(_context: any = {}, _data: any = {}): Promise<HttpResponse> {
        if (_context.testcaselib && true) {
            return this.http.post(`/testcaselibs/${_context.testcaselib}/testcaselibmodules/fetchroot_nobranch`, _data);
        }
    return new HttpResponse(null, { status: 404, statusText: '无匹配请求地址!' });
    }
}
