...
import { IPSAppDataEntity, IPSAppDEField, IPSAppDEListView,IPSAppDERS,IPSDEList, IPSDEListDataItem } from '@ibiz/dynamic-model-api';
...
export class ActionHistory extends AppListBase {
     ...
     * @memberof ActionHistory
     */
    public majorAppDataEntitys: Array<IPSAppDataEntity> = [];
     ...
     * @memberof ActionHistory
     */
    public getMajorAppDataEntity(){
        this.majorAppDataEntitys = [];
        ...
        if (appDataEntity && appDataEntity.major == false && minorAppDERSs.length > 0) {
            minorAppDERSs.forEach((minorAppDERSs: IPSAppDERS) => {
                const majorAppDataEntity: IPSAppDataEntity = minorAppDERSs.getMajorPSAppDataEntity() as IPSAppDataEntity;
                if (majorAppDataEntity) {
                    this.majorAppDataEntitys.push(majorAppDataEntity); 
                }
            });
        }
    }
     ...
     * 获取指定实体的属性名
     * 
     * @param entityCodeName 实体codename
     * @param fieldName 属性name
     */
    public getFieldText(entityCodeName: any, fieldName: any){
        let fieldText = fieldName;
        const majorAppDataEntity: IPSAppDataEntity | undefined = this.majorAppDataEntitys.find((majorAppDataEntity: IPSAppDataEntity) => Object.is(majorAppDataEntity.codeName?.toLowerCase(), entityCodeName.toLowerCase()));
        if (majorAppDataEntity) {
            const field: IPSAppDEField | undefined = majorAppDataEntity.getAllPSAppDEFields()?.find((item: IPSAppDEField) => Object.is(item.codeName?.toLowerCase(), fieldName.toLowerCase()));
            if (field) {
                fieldText = this.$tl(field.getLNPSLanguageRes()?.lanResTag, field.logicName);
            }
        }
        return fieldText;
    }
    ...
    protected renderHistoryItem(action: any, item: any): any {
        const fieldText = this.getFieldText(action.objecttype, item.field);
        if (item.diff) {
