 ...
 */
export default class TestRunUIServiceBase extends UIService {
     ...
     * @memberof  TestRunUIServiceBase
     */
    public dataService:TestRunService = new TestRunService();
    ...
    constructor(opts: any = {}) {
        super(opts);
        this.authService = new TestRunAuthService(opts);
        this.initViewMap();
    ...
    public async getRDAppViewEX(srfkey:string,isEnableWorkflow:boolean,rdviewname:string){        
        ...
        // 进行数据查询
        let result:any = await this.dataService.Get({testrun:srfkey});
