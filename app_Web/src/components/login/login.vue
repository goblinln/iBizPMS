...
export default class Login extends Vue {
    ...
    public async wxWorkHandleClick() {
        ...
        if(result?.state && Object.is(result?.state,"SUCCESS")){
            ...
            const redirect_uri_encode = encodeURIComponent(redirect_uri);
            // 3.钉钉扫码url
            const url = 'https://open.work.weixin.qq.com/wwopen/sso/qrConnect?state=STATE'
                + '&appid=' + appId
                + '&agentid=' + agentId
                + '&redirect_uri=' + redirect_uri_encode;
            // 4.跳转钉钉扫码
    ...
    public async WXWorkLogin(){
        ...
        const redirect_uri_encode = encodeURIComponent(redirect_uri);
        // 3.微信认证url
        const url = 'https://open.weixin.qq.com/connect/oauth2/authorize?response_type=code&scope=snsapi_base&#wechat_redirect'
            + '&appid=' + appId
            + '&redirect_uri=' + redirect_uri_encode
            + '&scope=snsapi_base'
            + '&state=STATE';
