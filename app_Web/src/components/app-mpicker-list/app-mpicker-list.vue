<template>
    <div class="app-mpicker-list-container">
        <i-select
            v-if="!hasChildren"
            class='app-mpicker-list'
            multiple 
            :transfer="true"
            transfer-class-name="app-mpicker-list-transfer"
            :value="currentVal"
            :disabled="disabled"
            :clearable="true"
            ...
            <i-option v-for="(item, index) in items" :key="index" :class="item.class" :value="item.value ? item.value.toString():''">
                <input type="checkbox" :checked="isSelect(item)">{{item.text}}
            </i-option>
        </i-select>
        <ibiz-select-tree v-if="hasChildren" class="tree-app-mpicker-list" :disabled="disabled" :NodesData="items" v-model="currentVal" :multiple="true"></ibiz-select-tree>
    </div>
</template>
...
export default class AppMpickerList extends Vue {
    /**
     * 代码表服务对象
     *
