...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/ibizpms-app-web\/ibzdocs\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibzdoc 方法: getwflink");
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/ibizpms-app-web\/ibzdocs\/process-definitions-nodes$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibzdoc 方法: getwfstep");
    console.table({url:config.url, method: config.method, data:config.data});
    console.groupEnd();
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
