...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/ibizpms-app-web\/companies\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/ibizpms-app-web\/companies\/process-definitions-nodes$/)).reply((config: any) => {
...
// FilterUpdate
mock.onPost(new RegExp(/^\/companies\/?([a-zA-Z0-9\-\;]{0,35})\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:company 方法: FilterUpdate");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const paramArray:Array<any> = ['id'];
    const matchArray:any = new RegExp(/^\/companies\/([a-zA-Z0-9\-\;]{1,35})\/fetchdefault$/).exec(config.url);
    let tempValue: any = {};
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
...
// FilterSearch
mock.onPost(new RegExp(/^\/companies\/?([a-zA-Z0-9\-\;]{0,35})\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:company 方法: FilterSearch");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterGet
mock.onPost(new RegExp(/^\/companies\/?([a-zA-Z0-9\-\;]{0,35})\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:company 方法: FilterGet");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterCreate
mock.onPost(new RegExp(/^\/companies\/?([a-zA-Z0-9\-\;]{0,35})\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:company 方法: FilterCreate");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterGetDraft
mock.onPost(new RegExp(/^\/companies\/?([a-zA-Z0-9\-\;]{0,35})\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:company 方法: FilterGetDraft");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterRemove
mock.onPost(new RegExp(/^\/companies\/?([a-zA-Z0-9\-\;]{0,35})\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:company 方法: FilterRemove");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterFetch
mock.onPost(new RegExp(/^\/companies\/?([a-zA-Z0-9\-\;]{0,35})\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:company 方法: FilterFetch");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
