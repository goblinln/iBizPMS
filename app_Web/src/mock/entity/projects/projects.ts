...
// FetchCurPersonalProjects
mock.onGet(new RegExp(/^\/projects\/fetchcurpersonalprojects$/)).reply((config: any) => {
    console.groupCollapsed("实体:project 方法: FetchCurPersonalProjects");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchCurPersonalProjects
mock.onGet(new RegExp(/^\/projects\/fetchcurpersonalprojects(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:project 方法: FetchCurPersonalProjects");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
...
// FetchCurUserTodo
mock.onGet(new RegExp(/^\/projects\/fetchcurusertodo$/)).reply((config: any) => {
    console.groupCollapsed("实体:project 方法: FetchCurUserTodo");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchCurUserTodo
mock.onGet(new RegExp(/^\/projects\/fetchcurusertodo(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:project 方法: FetchCurUserTodo");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
