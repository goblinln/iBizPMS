...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/pms-app-web\/projectteams\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/pms-app-web\/projectteams\/process-definitions-nodes$/)).reply((config: any) => {
...
// FetchProjectAllMembers
mock.onGet(new RegExp(/^\/projects\/([a-zA-Z0-9\-\;]{1,35})\/projectteams\/fetchprojectallmembers$/)).reply((config: any) => {
    console.groupCollapsed("实体:projectteam 方法: FetchProjectAllMembers");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const paramArray:Array<any> = ['id'];
    let tempValue: any = {};
    const matchArray:any = new RegExp(/^\/projects\/([a-zA-Z0-9\-\;]{1,35})\/projectteams\/fetchprojectallmembers$/).exec(config.url);
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
                enumerable: true,
...
// FetchProjectAllMembers
mock.onGet(new RegExp(/^\/projectteams\/fetchprojectallmembers$/)).reply((config: any) => {
    console.groupCollapsed("实体:projectteam 方法: FetchProjectAllMembers");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
...
// FetchProjectAllMembers
mock.onGet(new RegExp(/^\/projectteams\/fetchprojectallmembers(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:projectteam 方法: FetchProjectAllMembers");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
