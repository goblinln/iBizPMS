...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/ibizpms-app-web\/ibizproindices\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/ibizpms-app-web\/ibizproindices\/process-definitions-nodes$/)).reply((config: any) => {
...
mock.onGet(new RegExp(/^\/ibizproindices\/([a-zA-Z0-9\-\;]{1,35})\/select$/)).reply((config: any) => {
    ...
    let _items = items.find((item: any) => Object.is(item.indexid, tempValue.indexid));
...
mock.onPut(new RegExp(/^\/ibizproindices\/?([a-zA-Z0-9\-\;]{0,35})$/)).reply((config: any) => {
    ...
    mockDatas.forEach((item)=>{
        if(item['indexid'] == tempValue['indexid'] ){
...
// FetchESquery
mock.onGet(new RegExp(/^\/ibizproindices\/fetchesquery$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibizproindex 方法: FetchESquery");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchESquery
mock.onGet(new RegExp(/^\/ibizproindices\/fetchesquery(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibizproindex 方法: FetchESquery");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
...
// FilterUpdate
mock.onPost(new RegExp(/^\/ibizproindices\/?([a-zA-Z0-9\-\;]{0,35})\/fetchindexder$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibizproindex 方法: FilterUpdate");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const paramArray:Array<any> = ['indexid'];
    const matchArray:any = new RegExp(/^\/ibizproindices\/([a-zA-Z0-9\-\;]{1,35})\/fetchindexder$/).exec(config.url);
    let tempValue: any = {};
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
...
// FilterSearch
mock.onPost(new RegExp(/^\/ibizproindices\/?([a-zA-Z0-9\-\;]{0,35})\/fetchindexder$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibizproindex 方法: FilterSearch");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterGet
mock.onPost(new RegExp(/^\/ibizproindices\/?([a-zA-Z0-9\-\;]{0,35})\/fetchindexder$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibizproindex 方法: FilterGet");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterCreate
mock.onPost(new RegExp(/^\/ibizproindices\/?([a-zA-Z0-9\-\;]{0,35})\/fetchindexder$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibizproindex 方法: FilterCreate");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterGetDraft
mock.onPost(new RegExp(/^\/ibizproindices\/?([a-zA-Z0-9\-\;]{0,35})\/fetchindexder$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibizproindex 方法: FilterGetDraft");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterRemove
mock.onPost(new RegExp(/^\/ibizproindices\/?([a-zA-Z0-9\-\;]{0,35})\/fetchindexder$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibizproindex 方法: FilterRemove");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FilterFetch
mock.onPost(new RegExp(/^\/ibizproindices\/?([a-zA-Z0-9\-\;]{0,35})\/fetchindexder$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibizproindex 方法: FilterFetch");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
